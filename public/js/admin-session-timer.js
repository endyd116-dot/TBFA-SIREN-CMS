/* ============================================================
   ...
   방식(Swain 확정): 비활동(idle) 1시간. 활동(클릭·입력·이동)하면 유지,
   자리 비우면 1시간 뒤 만료. 5분 전(55분 무활동) 연장 팝업. 타이머/버튼 클릭 시 연장.
   - 우상단 타이머 = 무활동 남은 시간(활동하면 1:00:00로 리셋)
   - 활동 중에는 25분마다 JWT를 조용히 재발급(2시간 절대만료로 작업 중 끊기는 것 방지)
...
(function () {
  'use strict';
  var IDLE_MS = 60 * 60 * 1000;        // 1시간 무활동 → 만료
