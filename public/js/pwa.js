...
(function () {
  ...
  function injectStyle() {
    ...
    s.textContent =
      '#siren-pwa-banner{position:fixed;left:12px;right:12px;bottom:12px;z-index:9999;' +
      ...
      'display:flex;align-items:center;gap:12px;font-family:inherit;animation:sirenPwaUp .3s ease}' +
  ...
  function buildBanner(title, desc, btnLabel, onGo) {
    injectStyle();
    removeBanner();
    ...
    var ds = document.createElement('p'); ds.className = 'pwa-ds'; ds.textContent = desc;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (isiOS() && isSafari()) {
      ...
      setTimeout(function () {
        ...
        buildBanner('홈 화면에 추가', '하단 공유 버튼(￪)을 누른 뒤 "홈 화면에 추가"를 선택하세요.', null, null);
      }, 1500);
    }
  });
})();
