...
function jsonError(step: string, err: any) {
  return new Response(
    jsonKST({ ok: false, error: "기관 저장 실패", step, detail: String(err?.message || err).slice(0, 500), stack: String(err?.stack || "").slice(0, 1000) }),
    { status: 500, headers: { "Content-Type": "application/json" } }
  );
}

export default async (req: Request, _ctx: Context) => {
  if (req.method !== "POST") return new Response(jsonKST({ ok: false, error: "POST only" }), { status: 405, headers: { "Content-Type": "application/json" } });
  ...
  if (!name || typeof name !== "string" || !name.trim()) return new Response(jsonKST({ ok: false, error: "기관명은 필수입니다" }), { status: 400, headers: { "Content-Type": "application/json" } });
  if (!agencyType || typeof agencyType !== "string") return new Response(jsonKST({ ok: false, error: "기관 유형은 필수입니다" }), { status: 400, headers: { "Content-Type": "application/json" } });
  ...
  try {
    ...
    return new Response(jsonKST({ ok: true, id: resultId }), { status: 200, headers: { "Content-Type": "application/json" } });
