...
<p>긴 이야기이지만, 끝까지 함께해 주시길 부탁드립니다. 한 사람의 진심을 온전히 듣는 일은, 그 자체로 가장 따뜻한 연대입니다.</p>`,
    ...
    for (const seed of seeds) {
      const result = await db.execute(sql`
        INSERT INTO family_stories (title, category, duration, sort_order, summary, detail_html, status, created_at, updated_at)
        SELECT
          ...
          NOW()
        WHERE NOT EXISTS (
          SELECT 1 FROM family_stories WHERE title = ${seed.title}
        )
      `);
