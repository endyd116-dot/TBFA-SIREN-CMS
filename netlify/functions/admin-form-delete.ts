...
export default async (req: Request, _ctx: Context) => {
  if (req.method !== "POST" && req.method !== "DELETE") {
    return new Response(JSON.stringify({ ok: false, error: "POST/DELETE만" }),
  ...
  try {
    ...
    if (!row) return new Response(JSON.stringify({ ok: false, error: "없음" }),
    ...
    return new Response(JSON.stringify({ ok: true, deleted: row.title }),
