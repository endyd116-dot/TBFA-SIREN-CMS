...
async function handlePost(req: Request, adminId: number, adminMember: { role?: string | null }): Promise<Response> {
  ...
  if (typeof body.systemPrompt === "string") {
    ...
    if (!(await canAccess(adminMember.role || "", "ai_config_prompt"))) {
      return new Response(jsonKST({ ok: false, error: "AI 시스템 프롬프트 변경 권한이 없습니다" }),
        { status: 403, headers: JSON_HEADER });
    }
    const newPrompt = body.systemPrompt.trim();
    if (newPrompt.length < 30) {
      return new Response(jsonKST({ ok: false, error: "시스템 프롬프트가 너무 짧습니다 (30자 이상)" }),
        { status: 400, headers: JSON_HEADER });
    }
    if (newPrompt.length > 8000) {
      return new Response(jsonKST({ ok: false, error: "시스템 프롬프트가 너무 깁니다 (8,000자 이하)" }),
        { status: 400, headers: JSON_HEADER });
    }
    try {
      await setSystemPrompt(newPrompt, adminId);
      return new Response(jsonKST({ ok: true, target: "system_prompt" }),
        { status: 200, headers: JSON_HEADER });
    } catch (err: any) {
  ...
  if (typeof body.toolName === "string") {
    ...
    if (!toolName) {
      return new Response(jsonKST({ ok: false, error: "toolName 필수" }),
        { status: 400, headers: JSON_HEADER });
    }
    const patch: any = {};
    ...
    if (body.requiredRole !== undefined) {
      ...
      if (r === null || r === "" || r === "admin" || r === "super_admin") {
        ...
        return new Response(jsonKST({ ok: false, error: "requiredRole은 null|'admin'|'super_admin'" }),
          { status: 400, headers: JSON_HEADER });
      }
    }
    if (Object.keys(patch).length === 0) {
      return new Response(jsonKST({ ok: false, error: "변경 항목 없음" }),
        { status: 400, headers: JSON_HEADER });
    }
    ...
    try {
      ...
      return new Response(jsonKST({ ok: true, target: toolName }),
        { status: 200, headers: JSON_HEADER });
    } catch (err: any) {
  ...
  return new Response(jsonKST({ ok: false, error: "systemPrompt 또는 toolName 필요" }),
    { status: 400, headers: JSON_HEADER });
}
