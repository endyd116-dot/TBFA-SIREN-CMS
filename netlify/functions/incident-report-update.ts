...
export const config = { path: "/api/incident-report-update" };
...
export default async (req: Request, _ctx: Context) => {
  ...
  try {
    ...
    const [row]: any = await db
      ...
      .where(and(eq(incidentReports.id, id), eq(incidentReports.memberId, auth.uid)))
      .limit(1);
    ...
    try {
      await logUserAction(req, auth.uid, "user", "incident_report_update", {
        target: String(id),
        success: true,
      });
    } catch (_) {}
    ...
    console.error("[incident-report-update]", err);
