...
export default async function handler(_req: Request) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: true, durationMs: Date.now() - t0, summary }), {
      status: 200, headers: { "Content-Type": "application/json" },
    });
  } catch (err: any) {
    console.error("[cron-nurture] 실패", err);
    return new Response(JSON.stringify({ ok: false, error: String(err?.message || err).slice(0, 500) }), {
