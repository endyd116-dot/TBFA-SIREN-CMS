...
export default async function handler(req: Request, _ctx: Context) {
  ...
  function jsonError(step: string, err: any) {
    return jsonRes({ ok: false, error: "분기 오류", step,
  ...
  if (req.method === "GET") {
    try {
      ...
      return jsonRes({ ok: true, data: { quarters } });
  ...
  if (req.method === "POST") {
    ...
    try {
      const rows = await db.execute(sql`
        ...
        RETURNING *
      `);
      return jsonRes({ ok: true, data: { quarter: formatQ((rows as any).rows?.[0] || rows[0]) } }, { status: 201 });
    } catch (err: any) {
      if (err?.message?.includes("unique")) return jsonRes({ ok: false, error: "이미 존재하는 분기입니다" }, { status: 409 });
  ...
  if (req.method === "PATCH") {
    ...
    if (status && !VALID.includes(status)) {
      return jsonRes({ ok: false, error: "유효하지 않은 상태값" }, { status: 400 });
    }
    try {
      /* R32-P0-MS-C2 BUG fix: sql.raw(q, params) 파라미터 미바인딩 → sql 템플릿 합성 */
      if (!status && !settlementDate) {
        return jsonRes({ ok: false, error: "변경 필드 없음" }, { status: 400 });
      }
      ...
      return jsonRes({ ok: true, data: { quarter: formatQ(q) } });
