...
export default async (req: Request, _context: Context) => {
  try {
    ...
    /* ============================================================
       INSERT — 키 단위 존재 확인 후 안전 INSERT (재실행 안전)
       ============================================================ */
    let textInserted = 0;
    let textSkipped = 0;
    for (const item of seedTexts) {
      ...
      await db.execute(sql`
        INSERT INTO site_settings (key, value_text, has_draft, created_at, updated_at)
        VALUES (${item.key}, ${item.value}, false, NOW(), NOW())
      `);
      textInserted++;
    }
    ...
    for (const item of seedJsons) {
      ...
      await db.execute(sql`
        INSERT INTO site_settings (key, value_json, has_draft, created_at, updated_at)
        VALUES (${item.key}, ${jsonStr}::jsonb, false, NOW(), NOW())
      `);
      jsonInserted++;
    }
    ...
    const afterRes: any = await db.execute(sql`
      SELECT 
        ...
        (value_blob_id IS NOT NULL) AS has_blob,
        has_draft
      FROM site_settings
      WHERE key LIKE 'home.%'
      ORDER BY key
