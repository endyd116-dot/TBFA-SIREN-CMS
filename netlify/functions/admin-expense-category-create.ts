...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!code || !name) {
    return new Response(jsonKST({ ok: false, error: "code, name 필수", step: "validate" }), { status: 400 });
  }

  const codeStr = String(code).trim().slice(0, 32);
  if (!/^[a-z0-9_-]+$/i.test(codeStr)) {
    return new Response(jsonKST({ ok: false, error: "code는 영문·숫자·_- 만 사용 가능", step: "validate_code" }), { status: 400 });
  }
  ...
  if (existing.length) {
    return new Response(jsonKST({ ok: false, error: "이미 사용 중인 코드입니다", step: "duplicate_code" }), { status: 400 });
  }
