...
export default async function handler(req: Request, _ctx: Context) {
  ...
  if (!def) return jsonRes({ ok: false, error: "마일스톤을 찾을 수 없습니다" }, { status: 404 });

  // 카테고리 확인 (비매출만 카드 생성 가능)
  if (def.category === "REVENUE_LINKED") {
    return jsonRes({ ok: false, error: "매출 마일스톤은 카드 자동 생성을 지원하지 않습니다" }, { status: 400 });
  }
  ...
  if (def.target_milestone_role !== milestoneRole && member.role !== "super_admin") {
    return jsonRes({ ok: false, error: "본인 담당 마일스톤이 아닙니다" }, { status: 403 });
  }
