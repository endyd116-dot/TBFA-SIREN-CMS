...
function jsonError(step: string, err: any) {
  return new Response(
    jsonKST({ ok: false, error: "기관 비활성화 실패", step, detail: String(err?.message || err).slice(0, 500), stack: String(err?.stack || "").slice(0, 1000) }),
...
export default async (req: Request, _ctx: Context) => {
  ...
  const { id } = body;
  if (!id || isNaN(Number(id))) return new Response(jsonKST({ ok: false, error: "id는 필수입니다" }), { status: 400, headers: { "Content-Type": "application/json" } });
  try {
