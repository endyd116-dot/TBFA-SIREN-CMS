...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!id || refundAmount === undefined) {
    return new Response(jsonKST({ ok: false, error: "id, refundAmount 필수", step: "validate" }), { status: 400 });
  }
  if (Number(refundAmount) < 0) {
    return new Response(jsonKST({ ok: false, error: "환불금액은 0 이상이어야 합니다", step: "validate_amount" }), { status: 400 });
  }
  ...
  if (exp.status !== "approved") {
    return new Response(jsonKST({ ok: false, error: "승인된 항목만 환불 처리 가능합니다", step: "validate_status" }), { status: 400 });
  }

  // BUG-015 fix: 누적 환불 — 기존 refundAmount + 신규 합산, 누적합이 원금 초과 안 되도록 검증
  ...
  if (newTotalRefund > Number(exp.amount)) {
    return new Response(jsonKST({
      ok: false,
      error: `누적 환불액이 원금을 초과합니다. 기존 환불 ${currentRefund.toLocaleString("ko-KR")}원 + 신규 ${incremental.toLocaleString("ko-KR")}원 = ${newTotalRefund.toLocaleString("ko-KR")}원 > 원금 ${Number(exp.amount).toLocaleString("ko-KR")}원`,
