...
export default async function handler(req: Request, _ctx: Context) {
  ...
  function jsonError(step: string, err: any) {
    return jsonRes({ ok: false, error: "비매출 검증 오류", step,
  ...
  if (req.method === "POST" && action === "review" && idStr) {
    try {
      ...
      if (ach.status !== "PENDING") {
        return jsonRes({ ok: false, error: `현재 상태(${ach.status})에서 review 불가` }, { status: 400 });
      }
  ...
  if (req.method === "POST" && action === "verify" && idStr) {
    try {
      ...
      if (ach && !["PENDING", "REVIEWED"].includes(ach.status)) {
        return jsonRes({ ok: false, error: `현재 상태(${ach.status})에서 verify 불가` }, { status: 400 });
      }
      await db.execute(sql`
  ...
  if (req.method === "POST" && action === "reject" && idStr) {
    ...
    if (!rejectReason) return jsonRes({ ok: false, error: "반려 사유 필수" }, { status: 400 });
    try {
      const rows = await db.execute(sql`
  ...
  if (req.method === "PATCH" && action === "event-range" && idStr) {
    ...
    if (eventRangeAmount == null || !Number.isFinite(amt) || amt < 0) {
      return jsonRes({ ok: false, error: "eventRangeAmount는 0 이상 숫자여야 합니다" }, { status: 400 });
    }
    try {
      ...
      if (!md) return jsonRes({ ok: false, error: "비매출 성과를 찾을 수 없습니다" }, { status: 404 });
      ...
      if (isEventRange) {
        ...
        if (maxA > 0 && (amt < minA || amt > maxA)) {
          return jsonRes({ ok: false, error: `금액은 ${minA.toLocaleString()}~${maxA.toLocaleString()}원 범위여야 합니다` }, { status: 400 });
        }
      }
      await db.execute(sql`
