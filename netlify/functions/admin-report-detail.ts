...
function jsonError(step: string, err: any, status = 500) {
  return new Response(
    JSON.stringify({ ok: false, error: "보고서 상세 조회 실패", step, detail: String(err?.message || err).slice(0, 500) }),
...
export default async (req: Request, _ctx: Context) => {
  ...
  if (!id || id <= 0) return new Response(JSON.stringify({ ok: false, error: "id 파라미터 필요" }), { status: 400, headers: { "Content-Type": "application/json; charset=utf-8" } });
  ...
  if (!rows[0]) return new Response(JSON.stringify({ ok: false, error: "보고서를 찾을 수 없음" }), { status: 404, headers: { "Content-Type": "application/json; charset=utf-8" } });

  return new Response(
    JSON.stringify({ ok: true, data: { report: rows[0] } }),
