...
export default async function handler(req: Request, _ctx: Context) {
  ...
  try {
    ...
    if (!name || typeof name !== "string" || name.trim().length === 0 || name.length > 100) {
      return new Response(
        jsonKST({ ok: false, error: "템플릿 이름을 입력해 주세요. (1~100자)", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    if (!VALID_CHANNELS.includes(channel)) {
      return new Response(
        jsonKST({ ok: false, error: "채널 값이 올바르지 않습니다.", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    if (!VALID_CATEGORIES.includes(category)) {
      return new Response(
        jsonKST({ ok: false, error: "카테고리 값이 올바르지 않습니다.", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    if ((channel === "email" || channel === "inapp") && (!subject || !subject.trim())) {
      return new Response(
        jsonKST({ ok: false, error: "이메일·인앱 채널은 제목이 필요합니다.", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    if (!bodyTemplate || typeof bodyTemplate !== "string" || bodyTemplate.trim().length === 0) {
      return new Response(
        jsonKST({ ok: false, error: "본문을 입력해 주세요.", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    if (bodyTemplate.length > 10000) {
      return new Response(
        jsonKST({ ok: false, error: "본문은 10,000자 이하여야 합니다.", step: "validate" }),
        { status: 400, headers: JSON_HEADER },
      );
    }
    ...
    for (const v of variables) {
      if (typeof v.key !== "string" || typeof v.label !== "string" || typeof v.sample !== "string") {
        return new Response(
          jsonKST({ ok: false, error: "variables 각 항목은 key·label·sample 문자열이 필요합니다.", step: "validate" }),
          { status: 400, headers: JSON_HEADER },
        );
      }
    }
