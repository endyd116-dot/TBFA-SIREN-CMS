// netlify/functions/admin-legal-consultation-detail.ts
// M-10: 법률 상담 상세 + 답변 + 변호사 매칭

import type { Context } from "@netlify/functions";
import { eq, inArray } from "drizzle-orm";
import { db } from "../../db";
import { legalConsultations, members, blobUploads, reportStatusLogs } from "../../db/schema";
import { requireAdmin } from "../../lib/admin-guard";
import { sendEmail, tplLegalResponseUser } from "../../lib/email";
import { createNotification } from "../../lib/notify";
import { logAdminAction } from "../../lib/audit";
import {
  ok, badRequest, notFound, serverError,
  parseJson, corsPreflight, methodNotAllowed,
} from "../../lib/response";

export const config = { path: "/api/admin/legal-consultation-detail" };

const VALID_STATUSES = ["submitted", "ai_analyzed", "matching", "matched", "in_progress", "responded", "closed", "rejected"];

export default async (req: Request, _ctx: Context) => {
  if (req.method === "OPTIONS") return corsPreflight();

  const guard: any = await requireAdmin(req);
  if (!guard.ok) return (guard as { ok: false; res: Response }).res;
  const { admin } = guard.ctx;

  try {
    if (req.method === "GET") {
      const url = new URL(req.url);
      const id = Number(url.searchParams.get("id"));
      if (!Number.isFinite(id)) return badRequest("id 필요");

      const [row] = await db.select({
        consultation: legalConsultations,
        memberName: members.name,
        memberEmail: members.email,
        memberPhone: members.phone,
      })
        .from(legalConsultations)
        .leftJoin(members, eq(legalConsultations.memberId, members.id))
        .where(eq(legalConsultations.id, id))
        .limit(1);

      if (!row) return notFound("상담을 찾을 수 없습니다");

      const r: any = row.consultation;
      let attachments: any[] = [];
      if (r.attachmentIds) {
        try {
          const ids = JSON.parse(r.attachmentIds);
          if (Array.isArray(ids) && ids.length) {
            const files = await db.select().from(blobUploads).where(inArray(blobUploads.id, ids));
            attachments = files.map((f: any) => ({
              id: f.id, originalName: f.originalName, mimeType: f.mimeType,
              sizeBytes: f.sizeBytes, url: `/api/blob-image?id=${f.id}`,
            }));
          }
        } catch (_) {}
      }

      let responder = null;
      if (r.respondedBy) {
        const [resp] = await db.select({ id: members.id, name: members.name })
          .from(members).where(eq(members.id, r.respondedBy)).limit(1);
        responder = resp || null;
      }

      /* R41 Q2-002: 익명 신청은 신원(회원명·이메일·전화) 노출 차단 — 신원 식별은 admin-anonymous-reveal로만 (감사 기록) */
      const anon = !!r.isAnonymous;

      return ok({
        consultation: {
          ...r,
          // R45 CLUSTER-2(OP-007): 익명 상담 작성자 입력 신원 마스킹
          reporterName:  anon ? null : r.reporterName,
          reporterPhone: anon ? null : r.reporterPhone,
          reporterEmail: anon ? null : r.reporterEmail,
          memberName: anon ? null : row.memberName,
          memberEmail: anon ? null : row.memberEmail,
          memberPhone: anon ? null : row.memberPhone,
          attachments,
          responder,
        },
      });
    }

    if (req.method === "PATCH") {
      const body: any = await parseJson(req);
      if (!body) return badRequest("요청 본문이 비어있습니다");

      const id = Number(body.id);
      if (!Number.isFinite(id)) return badRequest("id 유효하지 않음");

      const status = body.status;
      const adminResponse = body.adminResponse !== undefined
        ? String(body.adminResponse).trim()
        : undefined;
      const assignedLawyerName = body.assignedLawyerName !== undefined
        ? String(body.assignedLawyerName).trim().slice(0, 50)
        : undefined;
      const sendMailFlag = body.sendEmail === true;
      const sendNotifyFlag = body.sendNotify !== false;

      if (status && !VALID_STATUSES.includes(status)) return badRequest("유효하지 않은 상태");

      const [row] = await db.select().from(legalConsultations).where(eq(legalConsultations.id, id)).limit(1);
      if (!row) return notFound("상담을 찾을 수 없습니다");

      const updateData: any = { updatedAt: new Date() };
      if (status) updateData.status = status;
      if (!status && adminResponse) updateData.status = "responded";
      if (adminResponse !== undefined) {
        updateData.adminResponse = adminResponse || null;
        if (adminResponse) {
          updateData.respondedBy = (admin as any).uid;
          updateData.respondedAt = new Date();
        }
      }
      if (assignedLawyerName !== undefined) {
        updateData.assignedLawyerName = assignedLawyerName || null;
        if (assignedLawyerName) {
          updateData.assignedAt = new Date();
        }
      }

      await db.update(legalConsultations).set(updateData).where(eq(legalConsultations.id, id));

      const [member] = await db.select({ id: members.id, name: members.name, email: members.email })
        .from(members).where(eq(members.id, (row as any).memberId)).limit(1);

      /* AD-014/015: 상태 전이 감지 — 답변 본문이 없어도 상태만 바뀌면 이력 기록 + 신청자 통지 */
      const prevStatus = (row as any).status;
      const statusChanged = !!status && status !== prevStatus;
      const STATUS_LABEL: Record<string, string> = {
        submitted: "접수", ai_analyzed: "AI 분석 완료", matching: "변호사 매칭 중",
        matched: "매칭 완료", in_progress: "상담 진행 중", responded: "답변 완료",
        closed: "종결", rejected: "반려",
      };
      const willNotify = sendNotifyFlag && !!member && (!!adminResponse || statusChanged);

      if (statusChanged) {
        try {
          await db.insert(reportStatusLogs).values({
            reportType: "legal",
            reportId: id,
            fromStatus: prevStatus,
            toStatus: status,
            changedBy: (admin as any).uid,
            note: adminResponse || (body.rejectionReason ? String(body.rejectionReason).slice(0, 500) : null),
            notifiedAt: willNotify ? new Date() : null,
          } as any);
        } catch (e) {
          console.warn("[admin-legal-consultation-detail] 단계 이력 기록 실패:", e);
        }
      }

      let emailSent = false;
      if (sendMailFlag && (adminResponse || statusChanged) && member?.email) {
        try {
          const tpl = tplLegalResponseUser({
            applicantName: member.name,
            consultationNo: (row as any).consultationNo,
            title: (row as any).title,
            newStatus: status || (row as any).status,
            assignedLawyerName: assignedLawyerName || (row as any).assignedLawyerName || null,
          });
          const result = await sendEmail({ to: member.email, subject: tpl.subject, html: tpl.html });
          emailSent = !!result.ok;
        } catch (e) {
          console.error("[admin-legal-consultation-detail] 메일 실패:", e);
        }
      }

      if (willNotify) {
        try {
          await createNotification({
            recipientId: member.id,
            recipientType: "user",
            category: "support",
            severity: "info",
            title: adminResponse
              ? "법률 상담에 답변이 등록되었습니다"
              : `법률 상담 처리 상태: ${STATUS_LABEL[status] || status}`,
            message: (row as any).title,
            link: `/mypage.html#support`,
            refTable: "legal_consultations",
            refId: id,
          });
        } catch (e) {
          console.warn("[admin-legal-consultation-detail] 알림 실패:", e);
        }
      }

      try {
        await logAdminAction(req, (admin as any).uid, (admin as any).name, "legal_consultation_response", {
          target: (row as any).consultationNo,
          detail: {
            status: updateData.status,
            hasResponse: !!adminResponse,
            assignedLawyer: assignedLawyerName || null,
            emailSent,
          },
        });
      } catch (_) {}

      return ok({
        id,
        consultationNo: (row as any).consultationNo,
        emailSent,
      }, sendMailFlag ? "답변이 등록되고 이메일이 발송되었습니다" : "답변이 등록되었습니다");
    }

    return methodNotAllowed();
  } catch (e: any) {
    console.error("[admin-legal-consultation-detail]", e);
    return serverError("처리 실패", e);
  }
};