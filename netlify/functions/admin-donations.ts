...
export default async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      /* paid_at 기준으로 통계 집계 — paid_at이 없는 구형 레코드는 COALESCE(hyosung_paid_date, created_at) 폴백 */
      const statsRes: any = await db.execute(sql`
        SELECT
          COALESCE(SUM(CASE WHEN status = 'completed' AND
            COALESCE(paid_at, CASE WHEN pg_provider ILIKE '%hyosung%' THEN COALESCE(hyosung_paid_date, created_at) ELSE created_at END) >= ${todayStartIso}::timestamp
            THEN amount ELSE 0 END), 0)::bigint AS today_amount,
          COALESCE(SUM(CASE WHEN status = 'completed' AND
            COALESCE(paid_at, CASE WHEN pg_provider ILIKE '%hyosung%' THEN COALESCE(hyosung_paid_date, created_at) ELSE created_at END) >= ${monthStartIso}::timestamp
    ...
    if (req.method === "PATCH") {
      ...
      if (body.refundOne === true) {
        ...
        /* autoRefundPg(신규)·autoRefundToss(레거시 호환) 둘 다 수용 */
        const autoRefundPg = body.autoRefundPg === true || body.autoRefundToss === true;
        const now = new Date();

        /* ── KICC 자동 환불 분기 ── */
        let pgRefundNote = "";
        if (autoRefundPg) {
          /* KICC 카드 결제만 자동 환불 가능 */
          const pg = (existing as any).pgProvider as string | null;
          const pgTid = (existing as any).pgTid as string | null;
          if (pg !== "kicc" || !pgTid) {
            return badRequest(
              `자동 환불은 KICC 카드 결제 + 거래번호(pgTid)가 있는 경우만 가능합니다 (현재 pg=${pg || "없음"}, pgTid=${pgTid ? "있음" : "없음"})`
            );
          }
          /* KICC 환불 API 호출 — 실패 시 DB 안 건드림 */
          const result = await cancelPayment({
            pgTid,
            reason: refundReason || `관리자 환불 (${admin.name || "관리자"})`,
          });
          if (!result.success) {
            return badRequest(
              `환불 실패: ${result.errorCode || "ERROR"} — ${result.errorMessage || "알 수 없는 오류"} (DB 상태 변경 없음)`
            );
          }
          pgRefundNote = ` [KICC 자동환불 OK status=${result.status || "?"} pgTid=${result.pgTid || "?"}]`;
        }

        const adminTag = `[환불 ${now.toISOString().slice(0, 10)} by ${admin.name || "관리자"}${pgRefundNote}]`;
        ...
        return ok(
          { donation: updated },
          autoRefundPg
            ? `₩${(updated.amount || 0).toLocaleString()} KICC 자동 환불 완료 (PG·DB 양쪽 반영)`
