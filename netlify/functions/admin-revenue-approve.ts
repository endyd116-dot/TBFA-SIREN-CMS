...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!(await canAccess(auth.ctx.member.role ?? "", "finance_bookkeeping"))) {
    return new Response(jsonKST({ ok: false, error: "재정 승인 권한이 없습니다", step: "auth_role" }), { status: 403 });
  }
  ...
  if (!id || !action) {
    return new Response(jsonKST({ ok: false, error: "id(또는 revenueId), action 필수", step: "validate" }), { status: 400 });
  }
