  ...
  try {
    ...
    const countRows = await db.execute(sql`
      ...
      WHERE member_id = ${memberId}
        ...
        AND (completed_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Seoul')::date >= (SELECT start_date FROM quarters WHERE id = ${quarterId})
        AND (completed_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Seoul')::date <= (SELECT end_date FROM quarters WHERE id = ${quarterId})
    `);
