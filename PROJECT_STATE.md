...
- **이어서 배포 2026-09-06.13**: 효성 통과 안전장치(옛 «수납대기»·청구목록 행이 «완납»을 되돌리지 않음·매칭 없는 행 안내) · 납부 확인서 PDF 글자별 배치(벌어짐 해결) · 결제 내역 관리 열 버튼 스타일 스크립트 주입(CMS)·상태/수단 라벨. 직인은 5월 카카오톡 사진(blob 9)이 등록돼 있음 → Swain이 영수증 설정에서 법인 직인으로 교체. 코드 `LANTERN`은 기본값·연동 주소만.
