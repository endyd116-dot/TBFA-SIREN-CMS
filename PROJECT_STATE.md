...
| 항목 | 값 |
|---|---|
| 진행률 | 🟢 B·A 평행 진행 중 — 코드 작성 중 |
| 담당 | 메인(설계 ✅) / B(백 — Sonnet 4.6) / A(프론트 — Sonnet 4.6) / C(검증 — Opus 4.7, B·A 머지 후) |
| 다음 할 일 | B push → 머지 → 마이그 호출 → schema 활성화 → A 머지 → C 검증 |
...
| 6순위 #16 단계 D | ✅ 코드 100% (1차: A 파서·B 화면 3a932c3 / 2차: A 매월 수납·B CSV 미리보기 머지 완료) / 🟡 라이브 검증 대기 (Q3) |
...
| ID | 발견 | 위치 | 심각도 | 상태 | 리포트 |
|---|---|---|---|---|---|
| #BACKFILL-1 | 2026-05-10 | 효성 후원 결제일 NULL (실제 44건 전체) | 🟡 Medium | 🟡 1차 마이그 가정 어긋남 (memo 패턴 매칭 0) — C 큐 Q-진단 추가 | [docs/issues/2026-05-10-hyosung-paid-date-backfill.md](docs/issues/2026-05-10-hyosung-paid-date-backfill.md) |
...
|---|---|---|---|---|---|
| `tbfa-mis` | **메인** | Opus 4.7 | 로직·DB 설계 + 머지·조율 | `docs/`, `PROJECT_STATE.md`, 머지 | 활성 — Phase 10 R1 머지 조율 중 |
| `../tbfa-mis-A` | **A** | Sonnet 4.6 | 프론트 구현 | `public/`, `assets/` | 🟢 활성 — Phase 10 R1 프론트 작성 중 (`feature/phase10-r1-front`) |
| `../tbfa-mis-B` | **B** | Sonnet 4.6 | 백 구현 | `netlify/functions/`, `lib/`, `db/`, `drizzle/` | 🟢 활성 — Phase 10 R1 백 작성 중 (`feature/phase10-r1-back`) |
| `../tbfa-mis-C` | **C** | Opus 4.7 | 라이브 검증 + fix + 백필 | 모든 영역 (검증·fix 한정) | ⏸ 대기 — Q3 또는 Phase 10 R1 검증 다음 세션 |
...
| # | 작업 | 종류 | 선행 조건 | 비고 |
|---|---|---|---|---|
| Q-진단 | #BACKFILL-1 추가 진단 (옵션 A) | 진단·마이그 갱신 | (없음) | 1차 진단 결과 가정 어긋남 (효성 44건 전부 NULL, memo 패턴 매칭 0건). memo 샘플 10건 조회 + 효성 import 코드 검증 + 정규식 보강 또는 fallback 컬럼 결정. [docs/issues/2026-05-10-hyosung-paid-date-backfill.md](docs/issues/2026-05-10-hyosung-paid-date-backfill.md) §"2026-05-10 진단 결과" |
...
| Q9 (예약) | Phase 10 R1 템플릿 빌더 검증 | 라이브 검증 | A·B 머지 완료 | 설계서 §4 Q1~Q8 시나리오 |

**완료**: ~~Q1~~ Phase 9-B 라이브 (2026-05-10 통과) / ~~Q2~~ #BACKFILL-1 마이그 작성 (2026-05-10 main 안착, Swain 호출 대기)
