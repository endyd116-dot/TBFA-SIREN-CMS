...
- **이어서 배포 2026-09-06.13**: 효성 통과 안전장치(옛 «수납대기»·청구목록 행이 «완납»을 되돌리지 않음·매칭 없는 행 안내) · 납부 확인서 PDF 글자별 배치(벌어짐 해결) · 결제 내역 관리 열 버튼 스타일 스크립트 주입(CMS)·상태/수단 라벨. 직인: 영수증 설정이 5월 카카오톡 사진(blob 9)을 가리키고 있어 확인서에 엉뚱한 이미지 → **배포 .14에서 설정을 근로계약 도장(blob 429 = assets/seals/seal-park-duyong.png)으로 바꾸고 코드 폴백(설정 없으면 저장소 도장)** · 옛 PDF 캐시는 19:00 KST 기준 재생성. 코드 `LANTERN`은 기본값·연동 주소만.
