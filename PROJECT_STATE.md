...
| 시각 | 갱신자 | 내용 |
|---|---|---|
| 2026-05-10 | 메인 | **Phase 9-B C 1차 머지** — notification_preferences + notification_admin_settings 테이블 마이그레이션 / FORCED_CHANNELS 도입 / 디스패처 사용자 설정 조회 통합 / 사용자 수신 설정 화면 + 어드민 전역 정책 화면 / 마이그레이션 호출 대기 |
...
🟢 Phase 9 진행 중
   ├─ A: ✅ SMS 어댑터 머지 (Aligo)
   ├─ B: ✅ 카카오 어댑터 머지 (Aligo) / ⏸ 카카오 심사 통과 후 템플릿 ID 환경변수 등록 → 실발송
   ├─ C: ✅ 9-B 1차 머지 / ⏸ Swain 마이그레이션 호출 대기 → 2차(schema 활성화·마이그 파일 삭제)
   └─ D: 휴면
   메인: 9-B 마이그 호출 안내 / 카카오 심사 통과 대기 / #BACKFILL-1은 9-B 마무리 후
```
...
| **Phase 9 외부 API 실연동 + 수신 설정 UI** | 🟢 A SMS·B 카카오 어댑터 머지 완료 / C 9-B 1차 머지 — 마이그레이션 호출 대기 / ⏸ 카카오 심사 통과·#BACKFILL-1 |
