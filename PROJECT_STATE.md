...
| 시각 | 갱신자 | 내용 |
|---|---|---|
| 2026-05-11 | 메인 | **Phase 10 R3 설계서 작성 완료 (발송 예약 큐 + 즉시 발송)** — DB(send_jobs + send_recipients 2테이블) / API 7개 + cron 1개(1분 단위 dispatcher, chunk 50건/회) / 화면 3개(목록·생성·상세 폴링) / 어댑터 직접 호출(Phase 8 이벤트 라우팅 우회) / 검증 시나리오 Q1~Q12 + cron 동작 + 성능 / 4채팅 프롬프트 / 평행 모드 / R2 머지 후 트리거 |
