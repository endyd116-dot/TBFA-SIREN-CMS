...
### 🧭 메뉴·페이지 통합 편집 개편 — 코드 완료 · **마이그 호출 대기** (2026-08-03·★최신)
...
- **미결(Swain 액션)**: 마이그 2회 호출 → ① `/api/migrate-site-pages?run=1`(저장소) ② `/api/migrate-page-content?run=1`(9페이지 이관+메뉴 연결). 이후 2차 푸시로 옛 주소 넘기기·마이그 파일 삭제.
- **푸시 2회 계획**: 마이그 호출이 라이브에서만 가능. 1차=1~8단계 전부, 2차=리다이렉트·정리.
