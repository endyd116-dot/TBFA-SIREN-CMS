...
- **이어서 배포 2026-09-06.12 — 등불 화면 «내용» 어드민 관리(§9·하드코딩 제거)**: 캠페인 관리 편집 창 «🕯️ 등불 테마·확장 설정»(라벨·부제·대표 한 줄·사다리·단체·FAQ 분류·증서·회칙 링크·OG) → `campaigns.extras`(jsonb). 시드 완료(컬럼 생성·캠페인 2 현재 문구 시드). 코드 `LANTERN`은 기본값·연동 주소만.
