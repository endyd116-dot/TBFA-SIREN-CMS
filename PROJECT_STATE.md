...
| 시각 | 갱신자 | 내용 |
|---|---|---|
| 2026-05-10 | **C 채팅** | **Phase 9-B 착수** — notification_preferences + notification_admin_settings 테이블 마이그레이션 함수 작성 / FORCED_CHANNELS 도입 / 디스패처 사용자 설정 조회 통합 / 사용자 알림 수신 설정 화면 + 어드민 전역 정책 화면 완성 / 마이그레이션 대기 중 |
...
```
🟢 Phase 9-B 진행 중 (2026-05-10)
   └─ C: Phase 9-B 사용자 수신 설정 UI + 어드민 전역 정책
         브랜치: feature/phase9-notify-preferences
         상태: 코드 완성 — 마이그레이션 호출 대기
         마이그: /api/migrate-phase9-notify-prefs?run=1 (어드민 로그인 후)
```
...
| **Phase 9 외부 API 실연동 + 수신 설정 UI** | 🟢 9-B 진행 중 — C 채팅 feature/phase9-notify-preferences / 마이그레이션 대기 |
