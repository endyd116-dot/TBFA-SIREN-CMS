...
### ⏭️ 다음 세션(압축) — 깜빡임 2건 근본 FIX + 납부 확인서 (2026-09-06·★최신 지시)
Swain: 캠페인 페이지가 밝은 화면 1초 뒤 등불 화면으로 바뀜 · 「함께하기」 모달이 떴다가 1초 뒤 내용이 바뀜 → **신뢰 훼손, 근본 FIX**. 원인(서버 렌더가 기본 레이아웃·클라가 fetch 뒤 교체 / 모달이 먼저 열리고 정책·회원상태를 기다린 뒤 교체)과 설계는 [`docs/active/2026-09-06-lantern-session-handoff.md`](docs/active/2026-09-06-lantern-session-handoff.md) §4. 트리거 = [`docs/active/2026-09-06-RESUME-TRIGGER.md`](docs/active/2026-09-06-RESUME-TRIGGER.md). 그다음 영수증 PDF→납부 확인서. Swain 몫: 효성 수납 16건 재매칭·통과, intent 3건 취소, 포트원 심사, 정관 업로드.
