...
| 시각 | 갱신자 | 내용 |
|---|---|---|
| 2026-05-15 | **메인** | **🎉🎉🎉 Phase 22-D 완결 + Phase 22 전체 마무리** (main @ `9e73e98`) — 22-D-R3 예산잠금·전표운영·재무제표 마감(C 검증 PASS 15/FAIL 3 → BUG-1~4 응답 키 불일치 fix). 예산 잠금·지출결의서 인쇄·결산 보조·이상패턴 배지·반복전표 cron(KST 04:30)·재정상태표·현금흐름표. **Phase 22 전체(22-A 매출 / 22-B 재정 3부작 / 22-C 지출 / 22-D 전표 3부작) 완결.** 다음: 함께워크 ON 또는 SIREN 안정화. |
| 2026-05-15 | **메인** | **🎉 Phase 22-D-R2 ✅ 마감** (main @ `fc547f1`) — 통장거래내역 자동화. IBK 엑셀 클라이언트 파싱 + 입출금 대사 엔진(묶음정산 합계대조·개별 후원매칭·계좌직접후원 회원후보·미매칭 분기) + 출금 전표 자동생성(AI 75%) + 거래처 마스터 자동학습. C 검증 15/0 + BUG-1 fix. |
| 2026-05-15 | **메인** | **🎉🎉 Phase 22-B 3부작 ✅ 완결** (main @ `c9f035a`) — 22-B-R3 NPO 표준 회계 보고서 마감(C 검증 12/0 + BUG-021). 운영성과표·예산실적표·인쇄/엑셀/PDF·옛 테이블 코드 제거. Phase 22-B(화면이전·예산편성·회계보고서) 3부작 완결. |
| 2026-05-15 | **메인** | **🎉 Phase 22-B-R2 + 22-D-R1 ✅ 마감** (main @ `7d080b8`) — 예산 편성·전표 시스템 두 라운드 동시 진행. C 통합 검증 + BUG-019/020 fix. AI 도구 그룹 갱신 의무 5곳 메모리 명문화. |
| 2026-05-15 | **메인** | **🎉 Phase 22-B-R1 ✅ 마감** (main @ `d28c833`) — 재정 화면 6개 통합 CMS 이전 + 지출 단일화 + 기간 필터. tsc 묵은 에러 14건 발견(배포 영향 없음·별도 정리 필요). |
...
```
🎉🎉🎉 Phase 22 전체 완결 (2026-05-15, main @ 9e73e98)
   ├─ 22-A 매출 통합 관리 ✅
   ├─ 22-B 재정 3부작 ✅ (R1 화면이전·기간필터 / R2 예산편성·결재 / R3 회계보고서)
   ├─ 22-C 지출 관리 ✅
   └─ 22-D 전표 3부작 ✅ (R1 전표 / R2 통장자동화 / R3 예산잠금·전표운영·재무제표)
   재정 시리즈 전체 운영 가능
...
⏸ 라이브 확인 권장 (Swain 직접)
   ...
   - DNS: www.tbfa.co.kr — 가비아 DNS에 www CNAME 추가 필요 (Netlify "propagating" 안 풀림)
```

**Swain 운영 액션** (작업 흐름 외):
- 카카오 심사 통과 후 환경변수 2개 등록 (ALIGO_TEMPLATE_BILLING_FAILED, ALIGO_TEMPLATE_CARD_EXPIRING) → 자동 발송
- www.tbfa.co.kr DNS — 가비아 DNS 관리에서 `www` CNAME → `tbfa-siren-cms.netlify.app` 추가
