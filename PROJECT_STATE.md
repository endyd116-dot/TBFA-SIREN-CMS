...
- **이어서 배포 2026-09-06.11 — 등불 가입자 «후원회원 등록 안내» 카톡 자동 발송 + 「등불 가입·미납」 후속 여정**(§8·Swain A안): 가입 직후 알림톡(검수 전엔 문자)·홈 `/?signup=1`이 가입 창(휴대폰 인증) 자동 열기·짧은 주소 `/lantern`·`/lantern/join`. **Swain 액션: `https://tbfa.co.kr/api/migrate-sponsor-welcome?run=1`(어드민) 1회 호출 → 알림톡 템플릿 등록·검수 신청 + 여정 시드 → 메인이 파일 삭제.** 여정 ON은 CMS 💌 후원자 너처링 › 예비 후원자 탭에서.
- **이어서 배포 2026-09-06.12 — 등불 화면 «내용» 어드민 관리(§9·하드코딩 제거)**: 캠페인 관리 편집 창 «🕯️ 등불 테마·확장 설정»(라벨·부제·대표 한 줄·사다리·단체·FAQ 분류·증서·회칙 링크·OG) → `campaigns.extras`(jsonb). 같은 시드 URL(`migrate-sponsor-welcome?run=1`)이 컬럼 추가+현재 문구 시드. 코드 `LANTERN`은 기본값·연동 주소만.
