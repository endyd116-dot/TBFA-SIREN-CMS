...
### ✅ 캠페인 페이지·후원 창 깜빡임 2건 근본 FIX — **배포 2026-09-06.9** (2026-09-06 압축 세션·★최신)
Swain: 캠페인 페이지가 밝은 화면 1초 뒤 등불 화면으로 바뀜 · 「함께하기」 모달이 떴다가 1초 뒤 내용이 바뀜 → 신뢰 훼손. **근본 FIX 완료**:
- 페이지: 서버가 등불 화면(실값·FAQ 포함)을 완성해 내보내고(`lib/campaign-page.ts`+`shell-detail`), 브라우저는 `data-ssr`가 있으면 다시 그리지 않는다(`#cmpData` 동봉). 다른 캠페인(제주)도 같은 방식.
- 후원 창: 페이지 로드 때 창을 미리 등불 상태로 만들고 회원 여부를 미리 조회(5분 캐시) → 클릭 즉시 최종 화면. 열기 리스너 capture 단계·`setTimeout(150)` 제거·미확정 때만 「준비 중」.
- 덤: AM 요청 ⑬(campaign-stats에 raisedKrw·goalKrw·donors·raisedAsOf) 같은 집계 함수에 additive 반영 → 회신 ⑧. 병합 도구 정리 커밋 동봉.
상세 = [`docs/active/2026-09-06-lantern-session-handoff.md`](docs/active/2026-09-06-lantern-session-handoff.md) §6.
- **이어서 배포 2026-09-06.10 — 영수증 PDF → 「후원금(회비) 납부 확인서」**(§7): 제목·항목명·각주(세액공제 아님·공익법인 지정 후 별도) 전환, 저장소의 옛 기본 문구는 코드가 «설정 안 함»으로 처리(DB 쓰기 0), 옛 서식 PDF 캐시는 자동 재생성, 마이페이지·후원 내역 표·메일·연간 안내 문구 정합. 공익법인 지정 시 되돌릴 자리 = `lib/pdf-receipt.ts` 상단.
Swain 몫: 효성 수납 16건 재매칭·통과, intent 3건 취소, 포트원 심사, 정관 업로드, 마이페이지에서 확인서 PDF 1회 열어 보기.
- **이어서 배포 2026-09-06.11 — 등불 가입자 «후원회원 등록 안내» 카톡 자동 발송 + 「등불 가입·미납」 후속 여정**(§8·Swain A안): 가입 직후 알림톡(검수 전엔 문자)·홈 `/?signup=1`이 가입 창(휴대폰 인증) 자동 열기·짧은 주소 `/lantern`·`/lantern/join`. **시드 완료(2026-09-06 Swain 호출 · 알림톡 템플릿 id 11 검수 중 · 여정 id 5 D+3/D+7 · 1회용 파일 삭제).** 여정 ON은 CMS 💌 후원자 너처링 › 예비 후원자 탭에서.
- **이어서 배포 2026-09-06.12 — 등불 화면 «내용» 어드민 관리(§9·하드코딩 제거)**: 캠페인 관리 편집 창 «🕯️ 등불 테마·확장 설정»(라벨·부제·대표 한 줄·사다리·단체·FAQ 분류·증서·회칙 링크·OG) → `campaigns.extras`(jsonb). 시드 완료(컬럼 생성·캠페인 2 현재 문구 시드).
- **이어서 배포 2026-09-06.13**: 효성 통과 안전장치(옛 «수납대기»·청구목록 행이 «완납»을 되돌리지 않음·매칭 없는 행 안내) · 납부 확인서 PDF 글자별 배치(벌어짐 해결) · 결제 내역 관리 열 버튼 스타일 스크립트 주입(CMS)·상태/수단 라벨. **AM 수신 ⑬(9/7)로 회신 ⑨ 전부 종결** — 실값 전환 완료·가입 안내는 SIREN 단독·미납 후속 SIREN 단독·extras 읽기는 다음 라운드. **운영 규칙: 어드민에서 등불 사다리·단체 표기·회칙 링크를 바꾸면 AM 메인에 통보.**
- 직인: 영수증 설정이 5월 카카오톡 사진(blob 9)을 가리키고 있어 확인서에 엉뚱한 이미지 → **배포 .14에서 설정을 근로계약 도장(blob 429 = assets/seals/seal-park-duyong.png)으로 바꾸고 코드 폴백(설정 없으면 저장소 도장)** · 옛 PDF 캐시는 19:00 KST 기준 재생성. 코드 `LANTERN`은 기본값·연동 주소만.

### 🧾 외부 등록(효성·일괄 등록) 정정 — **배포** (2026-09-06 새벽)
Swain "일괄 등록이 안 되고 신규 회원을 못 알아본다" → 원인 2개. ① 「신규 회원 일괄 등록」 [등록] 버튼이 서버 저장 없는 시연 코드(`실제 등록 시뮬레이션`) → `/api/admin-members-bulk` 실제 저장(전화번호 같은 기존 회원은 메모만 병합·가입경로 admin). ② 효성 계약/수납 매칭이 효성번호로만 → 효성번호 없는 기존 회원(홈페이지 가입 이현진 #66)을 두고 통과가 임시 계정(#102)을 또 만듦 → 적재·통과 모두 **전화번호 보조 매칭** 추가 + 미확정 목록 [선택 자동 재매칭](action `automatch`) + 중복 병합 도구 `/api/admin-member-merge?from=&into=(&run=1)`(어드민). 라이브 데이터: 수납 미매칭 10건(효성 63~67)·중복 이현진 102→66 병합은 **Swain이 CMS/도구로 실행**(메인은 삭제성 DB 조작 금지).
...
- **② 정적 페이지 15개 제목·설명 소실 회귀** — 홈 포함 라이브에서 `meta description` **0개**,
  제목이 전부 "SIREN | 교사유가족협의회"로 동일. 화면 파일엔 각각 제대로 있는데 `seo-injector`가
  원본 태그를 지우고 DB 값으로만 다시 쓰는데 정적 페이지는 DB 행이 없어 아무것도 못 넣었다.
  **2026-08-20 서버 렌더링 도입 때 생긴 회귀 — 이후 반려 3회 시점과 정확히 일치.**
- **F단계**: 홈 미션 소개 구역 신설(`home-intro` CMS 페이지 → page-shell 주입, sitemap 제외) +
  CMS 7종 확충(사진 있는 3종은 덧붙이기로 사진 보존) + 신규 안내 4종(유가족 첫 안내·순직 인정 절차·
  교권 보호 제도·투명한 운영) + 메뉴 4개 + FAQ 6건. **CMS 본문 4,600자 → 17,297자**
- **G단계**: `seo-injector`에 원본 메타 되살리기(`readOriginalMeta`) · 글꼴 미사용 굵기 제거
  (861KB/@font-face 1,144개 → 575KB/765개) · `javascript:void(0)` 링크 6곳 → 버튼 ·
  헤더 인증 확인 8회 → 2회 · 접근성(터치영역·대비·본문링크 밑줄) · 짧은 설명문 3개 보강
- **Lighthouse 모바일 실측**: SEO 85→**100** · 접근성 89→**96** · 성능 70→**84** · TBT 480→**140ms**
- **전수 검증**: 설명문 없는 페이지 **0/21** · 깨진 링크 0(Googlebot 70주소) · 자리표시 0 · 사이트맵 51개
- **다음**: Swain 재신청. 개선 여지 = Style&Layout 3초(한글 글꼴 @font-face 765개, 근본 해결은
  자체 호스팅 단일 subset)·활동 글·사진 꾸준한 추가

### 🌐 광고그랜트 재거부 대응 D·E단계 — **완료** (2026-09-03)
D단계(상세 서버 채움·시드)와 E단계(실제 사건 사례) 모두 마이그레이션 호출·검증 완료.
- **E단계**: 실제 사건 3건 시드 — 제주 현승준(2025·협의회 직접 대응 중)·대전(2023.9·순직 인정)·
  의정부 호원초 2인(2021·이영승 순직 인정/김은지 불인정·재조사 요구 계속). 전부 보도 사실만,
  미확정 날짜는 비움. + 서이초 정렬 + 자료실 메뉴를 공지사항 묶음으로 이동.
- **최종 전수 검증 25항목 실질 전부 통과** (2026-09-03): 사건 목록·상세 4건 본문, 목록 10종 서버 내용,
  상세 서버 채움, 사이트맵(새 사건 포함·가공 제외·테스트 글 제외·자료실 포함), 메뉴, 홈 505ms.
- **남은 것**: Swain이 구글 Ad Grants 활성화 요청 재제출. 미해결 1건 = 조직도 이메일
  endy0718@naver.com 통일 여부(Swain 결정). 마이그 파일 D·E 모두 삭제 완료.
(D단계 상세 — 위 블록으로 종결) 8-21 수정 후에도 3차 반려. 라이브 재진단으로 남은 원인 확정 + 전부 수정:
- **핵심 원인**: 사이트맵에 오른 **상세 페이지 40여 개가 제목만 있고 본문이 빈 껍데기**
  (활동·캠페인·사건·유가족 이야기·게시글·추모 상세 — 목록만 8-21에 채우고 상세는 빠뜨림).
- **가공 데모 사건 5건**이 실사이트에 사실처럼 게시 (부산·대구·인천·광주 = 실재 확인 불가, 본문 null).
  자유게시판 테스트 글("자유롭게"/박새로이)도 사이트맵 등재.
- **수정**: ① `lib/shell-detail.ts` 신설 — 상세 6종 서버 본문 채움 (`page-with-seo` 연결)
  ② 소식 FAQ·활동보고서 목록 서버 채움 추가 (`shell-lists`) ③ 자리표시 5곳 p 태그 정리
  (중첩 div면 replaceById 파손) ④ 검색 설명문의 "지도를 불러오는 중" 제거 ⑤ 자료실 복원
  (메뉴·사이트맵·noindex 해제 + 글 자료 열람 모달) ⑥ `migrate-adgrants-d` — 데모 사건 4건 비공개·
  서이초 본문·공지 5건·활동보고서 1건·자료 3건·조직도 검색설명·자료실 메뉴 시드.
- **다음**: Swain이 어드민 로그인 후 `https://tbfa.co.kr/api/migrate-adgrants-d?run=1` 호출
  → 성공 확인 → 마이그 파일 삭제 커밋 → 라이브 재검증 → 구글 재신청.

### 🌐 구글 광고그랜트(Ad Grants) 심사 대응 — **라이브 완료** (2026-08-20)
Ad Grants 활성화 요청이 두 번 반려된 원인을 진단하고 전부 해소. 구글 지적은 두 줄이었다 —
"더 빨리 로드되며 직관적인 탐색 환경 + 충분한 최신 콘텐츠·클릭 유도문안", "비영리 단체임을 확실히 알 수 있게 구성".

- **진짜 원인**: 상단 메뉴·단체 정보란·목록을 **화면이 열린 뒤 브라우저가 따로 받아 채우는** 구조.
  검색엔진·심사기관이 읽는 페이지 원본에는 메뉴도 단체 정보도 없었고, 운영자가 어드민에서 고친 값이
  그들에게 영영 전달되지 않았다. 게다가 예시값(대표 김◯◯·사업자번호 123-45-67890·모금 68%·지원 847건)이
  코드와 저장소 양쪽에 남아 있었다.
- **A단계(코드)**: 가짜 단체 정보 10개 파일 정정, 홈 특별 캠페인 예시 모금함 제거(어드민에서 켤 때만 그림),
  활동 지표를 실제 값으로, 검색엔진 단체 정보 형식 검증 추가
- **B단계(구조)**: `lib/shell-html.ts`(조립)+`lib/shell-render.ts`(조회)+`netlify/functions/page-shell.ts` 신설.
  공개 페이지 15종·콘텐츠 상세 6종·운영자 페이지(/p/*)를 **서버가 다 채운 상태**로 내보낸다.
  브라우저 재조회 방지값을 함께 심어 첫 화면 서버 왕복 7~10회 제거.
- **저장소 정정(1회용 도구 3종·호출 후 삭제)**: 정책 페이지 본문의 예시값, 개인정보 보호책임자
  (김광일/정책국장/010-7151-6883/endy0718@naver.com), 대표 연락처 통일(010-2807-5242 — 협회에 유선번호 없음)
- **결과(라이브 실측)**: 44개 페이지 전부 정상·깨진 링크 0건·가짜 정보 0건, 페이지 원본 글자수
  92~718자 → 22,000~44,000자, 홈 응답 1.1초+왕복 7~10회 → **0.72초**
- **주의**: 어드민 미리보기(`?preview=1`)는 서버가 채우지 않는다. 서버 조회 실패·2.5초 초과 시 예전 방식으로 안전 복귀.


### 🧭 메뉴·페이지 통합 편집 개편 — **라이브 완료** (2026-08-03)
"메뉴 하나 = 페이지 하나"로 바꾸고, 페이지 내용을 **통째로** 편집하게 만드는 개편. 화면 편집은 전부 메인 화면 편집으로 모으고, 콘텐츠 관리는 글(공지·자료실·게시판)만 담당. **단일 출처 = [`docs/active/2026-08-03-page-menu-redesign.md`](docs/active/2026-08-03-page-menu-redesign.md).**
- **해결한 문제 3가지**: ① 메뉴 화면에 상위/하위를 만들 방법이 아예 없었음(납작한 표) ② 협의회 소개 3개 메뉴가 전부 같은 파일의 다른 위치로 스크롤만 함 ③ 페이지 뼈대가 코드에 박혀 빈칸 8개만 채울 수 있었음
- **새로 만든 것**: 페이지 저장소(본문·임시저장·되돌리기 20개)·페이지 API 3종·본문 정화(허용목록·검증 19종)·썬에디터 통 편집기(글자색·정렬·크기·표·사진크기·유튜브)·페이지 관리 화면·**메뉴 드래그 2단 트리**·공개 페이지 `/p/{주소}`(서버 렌더·검색노출)
- **자동화**: 메뉴 추가 시 페이지 자동 생성·연결, 한글 제목 → 영문 주소 자동 변환, 페이지를 가리키는 메뉴는 주소 자동 생성(숨김·삭제 페이지는 공개 메뉴에서 자동 제외)
- **마이그 2회 호출 완료**(저장소 준비·9페이지 이관). 1회용 파일 삭제 완료. 옛 주소 넘기기(약관류 4종 301 + 소개·활동 화면단) 적용 완료.
- **배포 후 실측으로 잡은 3건**: ① 저장소에 '주요 연혁'·'조직도' 메뉴가 없어 페이지 접근 불가 → 없는 메뉴 자동 생성 ② `/p/` 전부 404(넘김 규칙의 `*` 패턴이 함수로 안 감) → 함수가 경로 직접 수신 ③ 검색 설명에 자리표시 노출 → 치환 후 본문에서 추출.
- **라이브 검증**: 새 페이지 9종 200 · 옛 주소 4종 301 · 자리표시 0건 · 상단 메뉴 회귀 없음.
- **남은 확인(Swain)**: '조직도' 메뉴가 노출 OFF 상태 — 메뉴 관리에서 켜면 나타남. 편집기·드래그·지도 화면 확인.

### 📝 직원 전자 근로계약 시스템 — 라이브·E2E 통과 (2026-07-27·★최신)
이사장이 계약서 작성→발송(회사 도장 자동날인)→직원이 워크스페이스에서 인적사항 입력+서명→R2 박제→양측 다운로드. **단일 출처 = [`docs/active/2026-07-27-session-handoff.md`](docs/active/2026-07-27-session-handoff.md).**
- **DB 5테이블**(사업자·양식·계약본체·증적·부속서류) + 프리셋 3사(교유협·ON·AX)·박두용 도장 자동등록. 마이그 호출·삭제 완료.
- **주민번호 AES-256-GCM 암호화**(env `CONTRACT_PII_KEY` 등록 완료)·화면 마스킹·**직원이 서명 시 입력**(이사장 아님). 인적사항(생년월일·주소·연락처) 전부 직원 작성·미치환은 밑줄 렌더.
- **사업자별 양식 CRUD**·수습 지급률(%) 선택(기본90·김주안100)·초안 수정/삭제·5인미만 안전 조항(무효위험 4개 다듬음).
- 통합 CMS iframe 4곳 등록(super 전용 `contract_manage`)·워크스페이스 "내 근로계약". E2E 라이브 통과.
- **미결(다음 메인)**: ① 업데이트 소식 상세 보기(Swain 요청·미착수) ② 근로계약 종결(알림·메뉴얼·release_checklist). 김주안 계약은 Swain 직접 생성 중.

### 🧾 지출결재·지출결의서 트랙 — 라이브 (2026-07-16~20)
지출 결재 워크플로우를 실사용 수준으로 완성. 전부 main 배포 완료. 상세·재발주의 = `docs/rules/HANDOFF.md` §0-A.
- **7/16**: 증빙 첨부 + 결재라인 CRUD(이사장 전용·화면에서 결재선 편집) / 지출결의서 결재취소·기안삭제·발행본 일괄다운로드·검색·CSV / 반려건 재상신 + 결재 지연 리마인드 cron(`cron-approval-delay-reminder`) / 예산과목(관·항·목) CRUD 버튼 무반응 fix / 협의회 소개 시드 마이그.
- **7/20**: 증빙이 안 열리던 P1 버그 fix(업로드 3단계 중 **완료확정 누락** → `blob_uploads.upload_status`가 pending으로 굳어 425 서빙거부·파일은 R2에 정상). 굳은 4건 복구 완료(`healedCount:4`). 지출결의서 PDF 자기결재칸 제거·기안일 표기·본문 표 넘침 fix·발행본 재발행(이사장 전용). 생성 로직 `lib/resolution-pdf-store` 공용화.
- **마이그**: `migrate-about-content` **Swain 호출 완료**(seeded 6키: 인사말·서명·비전3카드·연혁)·파일 삭제 완료. `migrate-heal-pending-receipts` 호출·삭제 완료.
- **잔존 1회용 마이그 2개**(7/13 이전 잔여·호출여부 미확인): `migrate-content-perm`·`migrate-r44-reindex`.
- **APP_VERSION** `2026-07-20.1`·소식 초안 7건 추가(발행은 운영자 수동).

### 💰 급여 시스템 고도화 + 🕐 KST 전면 정비 — 라이브 (2026-07-11~13)
**단일 출처 = [`docs/active/2026-07-13-session-handoff.md`](docs/active/2026-07-13-session-handoff.md)** (본 절은 색인만).
- **급여**: 근태 정정이 재집계에 반영 안 되던 버그(+795,455원 회복)·소득세 0원 문제(국세청 간이세액표 코드 embed `lib/income-tax-table.ts`)·주말/공휴일 출근 급여 제외·법정 휴게시간 단계화. **지급일수를 실근무시간 기준으로 전면 개정**(8h↑1.0/6~8h 0.75/4~6h 0.5/2~4h 0.25).
- **신규**: 전자서명·증빙보관(R2 sha256 고정)·직책·조정항목 과세/비과세·**법정 신고 서류**(임금대장·원천징수·간이지급명세서·연간집계·4대보험, 주민번호 미저장)·[지급] 워크플로우 가드·재택보고서 미제출 근무 불인정·근태 수정요청 파일 첨부.
- **KST**: 단일 출처 `lib/kst.ts`/`public/js/lib-kst.js`. 저장·전송은 UTC, 변환은 표시·날짜판정 시만. API 응답은 반드시 `jsonKST()`/`jsonRes()` 관문 통과. **🚫 서버·DB 세션 TZ를 Asia/Seoul로 바꾸는 것 절대 금지**(모든 시각 9시간 파괴·실측). '날짜' vs '시각(instant)' 구분 필수.
- 덤으로 복구: 휴가 신청 내역 조회·전표 목록·신고 게시판 500 / 워크스페이스 7개 화면 사용자정보 빈 껍데기.

### 📌 미결 — Swain 액션 (2026-07-20 기준)
1. 🔴 **간이지급명세서 제출 — 기한 7/31**(급여관리 → 신고·법정 서류).
2. 🔴 **관리자 비밀번호 변경**(git 히스토리 노출 이력·변경만이 해결).
3. 김광일 6월 명세서 마무리(서명 → [지급] 확정) / 업데이트 소식 초안 발행.
4. 🔎 **공유 캘린더 일정이 특정 사용자 화면에서 흰색으로 안 보임**(2026-07-20 제보·김주안) — 원인 규명 중. 본 프로젝트에 다크모드 CSS 0개라 브라우저 다크모드·확장 프로그램 유력 + 정의되지 않은 일정 종류 폴백 부재(작업 카드는 2026-05-16 동일 결함 fix 이력 있음·`workspace-calendar.css:131`).

---

### 🛡️ [archive] R45 운영 시작(06.01) 전 최종 점검 — 역할 4티어 전수 감사 + 수정 라운드 (2026-05-29 최신·★상단)
...
- **P4 종결 정리(2026-05-27)**: ⑴ AI 비서 안내문(`FALLBACK_SYSTEM_PROMPT`)에 딥릴리프 도메인+읽기 도구 3개 정식 정착 ⑵ 발간 쓰기 권한을 권한 정책(`role_permissions.martyrdom_publication`·cms 탭)과 **실제 연동**(`canAccess`)+화면 버튼 정책 반영(`canWrite`·기본 admin ON/operator OFF로 현행 보존) ⑶ 메뉴얼·AI 학습자료(P3 서면+P4) 동기화 ⑷ 설계서 milestone 이동·종결 선언. **✅ Swain 발간 권한 마이그(`migrate-martyrdom-pub-perm`) 호출 완료(시드 1행·1회용 파일 삭제)·권한 정책 cms 탭 등록.**
- **종결 후 자료 추출 개선(2026-05-27·Swain 요청)**: ⑴ m4a 전사 — mime 후보 폴백(`audio/mp4` 거부 시 `audio/aac`) ⑵ 음성·영상 **전사 전문 열람** — 자료 [보기]가 요약 1줄→추출/전사 전문 표시(on-demand `admin-martyrdom-doc-text`·복사 버튼·원본 삭제된 미디어의 유일 열람 수단) ⑶ 구형 **.doc** 본문 추출(`word-extractor`·신규 dep·external 등록) ⑷ **.hwp** 본문 전체 추출 강화(BodyText 레코드 파싱·cfb+zlib·실패 시 PrvText 폴백). tsc 0·캐시버스터 admin-martyrdom.js P4e.
...
- **대용량 음성·영상 Files API + 원본 삭제(`2fd765e`·`dfa65ec`·Swain 요청)**: 인라인 15MB 가드 제거 → ai-gemini `uploadToGeminiFiles`/`deleteGeminiFile`+`fileParts`로 Gemini Files API 전사(53~75MB+ 처리·메모리상한 350MB). 업로드 100→300MB. extract-background는 미디어를 bytes로 직접 전사(base64 회피)하고 **전사 성공 시 R2 원본 deleteFromR2+blob_key NULL**(저장공간 절약·텍스트로 분석). 원본 삭제된 미디어 [재처리]는 저장 전사 텍스트로 재분류. viewDoc은 '원본 전사 후 삭제' 안내. 캐시버스터 v9. ⚠️ m4a(audio/mp4) Gemini 거부 시 ffmpeg 변환 검토 필요(미결).
...
- 라벨: 웹 가입경로 → **'싸이렌웹'**(`69ca729`). 과거 NULL 회원 백필 + 라벨 통일 = 1회용 마이그. **✅ Swain 호출 완료**(backfilledTo:1·`싸이렌 홈페이지`→`싸이렌웹`·remaining 0)·파일 삭제됨.
...
1. **딥릴리프 P1 재검증(2차 배포 후)**: [전체 재시도] → 엑셀·이미지·텍스트 모두 추출·분류 되는지. PASS면 P1 종결. 잔여 실패는 요약칸 빨간 사유로 추가조치. (혹시 '비용 관리'에서 martyrdom_ai 켜짐 확인.)
2. ✅ A 마이그 호출 완료(백필·싸이렌웹·파일 삭제). **A·B 라이브 확인**: 가입경로 '싸이렌웹' 표시·신규 웹 가입 집계·알림·발송 그룹의 '알림 발송 로그' 메뉴.
3. **P2 설계·분배** (P1 PASS 게이트): 설계서 §5.3·§6 — ③전략·①골든타임·②인정요건대조·⑨모순·⑩타임라인+공백·⑪반론·⑫준비도+G3+코퍼스검색·동의/보존 → §6 P2 트리거 추가 후 B·A·C 분배.
- 메모리 `project_next_survivor_support`(§0 결정·Deep-Relief 정합).
- **잔여 정리**: exceljs npm audit 경고(transitive) 모니터.
