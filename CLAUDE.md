...
> 상세 인수인계서는 [`docs/history/handover/v20.md`](docs/history/handover/v20.md) 참고.
> 문서 구조 진입점: [`docs/README.md`](docs/README.md) (2026-05-20 4영역 분리 — rules/specs/active/history)
...
| 운영 주체 | (사)교사유가족협의회 (사업자번호 3818200754) |
...
└── docs/                           ← 4영역 분리 (2026-05-20)
    ├── README.md                       문서 구조 진입점
    ├── rules/                          운영·작업 규칙 (HANDOFF·REMAINING_WORK·PARALLEL·CONTEXT·policies·standards)
    ├── specs/                          명세 마스터 7개 (근태·성과·재정 + phase24·26·27·28)
    ├── active/                         진행 중 라운드 (현재 비어있음)
    └── history/                        완료 히스토리 (milestones·verify·gap·analysis·diagnostics·cleanup·handover·통합 archive 3종)
```
...
사용자가 명시한 자율 권한:
- **자동 진행**: 코드 수정, git add/commit, npm 명령, 옵션 추천 진행, 메모리 갱신
  - `git push`·Netlify 배포는 **§9.3 푸시 배치 정책**을 따른다 — 자유 push 아님. 검증 단위로 모아서만.
...
### 6.17 A·B·C 채팅 자율주행 (2026-05-14 신설 · 2026-05-21 push 금지로 개정)

A·B·C 서브 채팅은 **자기 worktree에서 commit까지만 자율, push는 안 한다.** 완료되면 메인에 "머지 요청"하고, **메인이 여러 작업을 모아 한 번에 main 머지·push**한다 (§9.3 배포 비용 절감 — push=배포 과금).

- **자율 (allow)**: Read·Edit·Write 모든 파일, git status/log/diff/fetch/add/**commit**/rebase, bash·PowerShell 일반 명령, npm install/run
- **금지 (deny)**: **`git push` (메인 단독 — A·B·C는 절대 push 안 함)**, force push, hard reset, rm -rf, lib/auth.ts·admin-guard.ts·hyosung-parser.ts 수정, public/js/auth.js·admin-mypage-cancellation.js·admin-eligibility.js 수정
- **묻기 (ask)**: 설계·로직 결정, package.json/lock 수정, npm uninstall/update, netlify/curl
- **완료 보고**: 작업 끝나면 브랜치명·커밋 해시·변경 요약을 메인에 전달 → 메인이 `git fetch` 후 머지. 라이브 검증이 필요하면 **메인이 다른 완료 작업과 묶어 1회 push.**

정책 위치: `.claude/settings.json` (메인+A+B+C 워크트리 4곳·**A·B·C는 push deny**) + 트리거 본문 `[자율주행 정책]` 조항. 메인 폴더 settings만 push 가능.

**Why**: 매 Edit/Read마다 허락 묻기 → 작업 흐름 끊김. A·B 효율 급감. 2026-05-14 Swain "허락 맡지마" 명시 지시.

### 6.18 모든 웹페이지 콘텐츠·기능은 싸이렌 어드민에서 관리 (2026-05-24 Swain 추가)

신규 공개 페이지·기능을 만들면 **반드시 싸이렌 어드민(통합 CMS `cms-tbfa.html`)에 운영자 관리 화면을 함께 제공**한다. 운영자가 코드 없이 콘텐츠·노출·발행을 관리할 수 있어야 한다.

- **필수**: 공개 페이지의 동적 콘텐츠(목록·상세·카드·배너·문구 등)는 DB + 어드민 CRUD로 관리. **코드 하드코딩으로만 존재하는 콘텐츠 금지.**
- **등록**: 어드민 화면은 `cms-tbfa` iframe 4곳 등록(code_standards #4: 사이드바·section·tabLabels·탭분기)으로 **싸이렌 어드민 안에서** 열리게 한다. 통합 CMS 밖의 분리된 어드민 페이지로 두지 말 것.
- **시드**: 초기 콘텐츠를 코드/마이그로 넣더라도, 이후 **수정·추가·삭제·발행은 어드민에서** 가능해야 한다.
- **체크**: 라운드 종결 체크리스트(release_checklist #9 iframe 4곳·#1 권한 정책)에 포함.

**Why**: Swain은 비개발자 운영 결정자. 웹페이지 기능·콘텐츠를 코드로만 관리하면 매번 개발자(Claude) 호출 필요 → 운영 자립 불가. "웹페이지 관련 기능 수정은 무조건 싸이렌 어드민에서 관리"가 원칙(2026-05-24 재확인).
...
### 9.3 푸시 정책 — 배포 비용 절감 (2026-05-21 전면 개정·Swain 지시)

**원칙: `git push` 1회 = Netlify production 배포 1회 = 크레딧 과금.** (2026-05-21 한 달 1,426배포로 비용 폭증 인시던트 — Production deploys가 크레딧 79% 소모.) 상한(auto recharge 한도)은 정상 push까지 막을 위험이 있어 걸지 않고, **워크플로우로 push 횟수를 최소화한다.**

1. **배치 push (핵심)**: 작업 중 `commit`은 자유롭게 누적, **`push`는 "Swain 라이브 검증이 꼭 필요한 단위"가 완성됐을 때만 1회.** 중간 진행·리팩터·내부 단계는 push하지 말고 commit으로 쌓는다.
2. **단계별 push 최소화**: 큰 라운드라도 단계마다 push 금지. **Swain이 단계별 라이브 검증을 명시 요청한 경우에만** 그 경계에서 push, 그 외 다단계는 전부 commit 누적 후 마지막 1회 push.
3. **문서·환경변수 변경 동봉**: `PROJECT_STATE`·`HANDOFF`·`docs` 갱신은 **단독 push 금지**, 다음 코드 push에 묶는다. 환경변수 변경도 모아서 한 번에(재배포 1회).
4. **push 주체 = 메인 단독**: A·B·C 서브 채팅은 push 안 함(§6.17). 완료 시 메인에 머지 요청 → **메인이 여러 작업 브랜치를 모아 한 번에 main 머지·push.**
5. **병렬 라운드 베이스 push 생략 (2026-05-26·워크트리 공유)**: A·B·C가 **같은 로컬 `.git` worktree**면(현 환경), 라운드 베이스를 origin에 push할 필요 없이 **로컬 `main`에서 분기/rebase**한다(설계·마이그를 로컬 main commit → 트리거가 `main` 지정). 공유 객체저장소라 베이스 일치 보장 + 분배 시 **배포 0**. push는 **B 머지(마이그 라이브 호출)·A 머지(라이브 검증)** 등 라이브 필수 시점만. (cross-machine·repo 분리면 종전대로 origin push — PARALLEL_GUIDE §4.1.)
6. **즉시 push 예외**: 운영 장애 핫픽스(사이트 다운·결제·인증·발송 장애 등 사용자 긴급 영향)만. 그 외는 모두 배치.
7. **문서·주석만 바뀐 push는 `[skip netlify]` (2026-06-13 팀 공통 규칙·AM도 동일 적용)**: 코드 영향 0인 push(문서·주석·메모만)는 **push의 마지막(HEAD) 커밋 메시지**에 `[skip netlify]` 포함 — Netlify가 빌드 자체를 스킵해 크레딧 0. ⚠️ Netlify는 push의 최신 커밋 메시지만 검사하므로, **코드 커밋과 문서 커밋을 함께 배치 push할 때는 마지막 커밋에 절대 넣지 말 것**(코드 배포까지 통째로 스킵됨).
8. **빈발 크론 금지 — DB 상시가동 방지 (2026-06-13)**: 빌트인 Netlify DB(Neon)는 5분 유휴 시 autosuspend되는데, 5분 미만 간격 크론은 DB를 영원히 깨워둠(2026-06 Database compute 3,306cr 인시던트 — 1분 크론 2개가 원인). 신규 크론은 **최소 10분 간격 + 기존 빈발 크론과 같은 분(minute)에 정렬**(:00/:10/:20/:30/:40/:50). 1~5분 주기가 꼭 필요하면 Swain과 사전 협의.
...
[docs/rules/PAGES.md](docs/rules/PAGES.md) — 사용자·어드민·워크스페이스 진입점 카탈로그.
...
| [`docs/README.md`](docs/README.md) | **문서 구조 진입점** (rules/specs/active/history 4영역) | 구조 변경 시 |
| [`docs/rules/HANDOFF.md`](docs/rules/HANDOFF.md) | 단일 최신 인수인계 (한 화면) | 항상 단일 최신 |
| [`docs/rules/PARALLEL_GUIDE.md`](docs/rules/PARALLEL_GUIDE.md) | 병렬 작업·머지·충돌 회피 가이드 | 정책 변경 시 |
| [`docs/rules/PARALLEL_TEMPLATE.md`](docs/rules/PARALLEL_TEMPLATE.md) | 트리거 템플릿 | 정책 변경 시 |
| [`docs/rules/PAGES.md`](docs/rules/PAGES.md) | 페이지 진입점 카탈로그 | 페이지 추가 시 |
| [`docs/rules/REMAINING_WORK.md`](docs/rules/REMAINING_WORK.md) | 잔여 작업 인벤토리 | 우선순위 합의 시 |
| [`docs/rules/CONTEXT_OPTIMIZATION.md`](docs/rules/CONTEXT_OPTIMIZATION.md) | 컨텍스트 다이어트 진단·결정 | 다이어트 정책 변경 시 |
| [`docs/rules/policies/roles-and-permissions.md`](docs/rules/policies/roles-and-permissions.md) | 4계층 권한 정책 | 권한 정책 변경 시 |
| [`docs/rules/standards/AI_AGENT_PLATFORM_STANDARD.md`](docs/rules/standards/AI_AGENT_PLATFORM_STANDARD.md) | AI 에이전트 표준 | 표준 갱신 시 |
| [`docs/specs/`](docs/specs/) | 명세 마스터 7개 (근태·성과·재정 + phase24·26·27·28) | 명세 변경 시만 |
| [`docs/active/`](docs/active/) | 진행 중 라운드 (현재 비어있음) | 새 라운드 신설 시 |
| [`docs/history/`](docs/history/) | 완료 히스토리 (milestones·verify·gap·analysis·diagnostics·cleanup·handover·통합 archive 3종) | append-only |
...
| `docs/README.md` | **세션 시작 시 진입점** (4영역 구조 확인용·짧음) |
| `docs/rules/HANDOFF.md` | **§3·§5·§7만 발췌** (limit/offset 사용, 전체 정독 X) |
| `docs/rules/PARALLEL_GUIDE.md` | **새 병렬 작업 분배 시에만** |
| `docs/rules/PARALLEL_TEMPLATE.md` | 새 라운드 설계 시 정독 |
| `docs/rules/PAGES.md` | 페이지 위치 헷갈릴 때만 |
| `docs/history/handover/v*.md` | **자발적 안 읽음** (영구 archive) |
...
- 인수인계는 [`docs/rules/HANDOFF.md`](docs/rules/HANDOFF.md) 갱신 후 새 채팅 시작 메시지로 위치 안내

### 14.5 다이어트 결과 요약

본 정책 도입 시점(2026-05-10) 자동 로드 약 28% 절감. 자세한 진단·결정 [docs/rules/CONTEXT_OPTIMIZATION.md](docs/rules/CONTEXT_OPTIMIZATION.md).

---

**마지막 업데이트**: 2026-05-20 (R29~R37 명세 정합 시리즈 종결 + docs/ 4영역 분리(rules·specs·active·history) + 경로 통일)

이전 업데이트:
- 2026-05-12 §9.1.9 사전 정독 + §13 체크리스트 2개 + §14.1 memory 정독 + §6.15 진행 중 채팅 알림 의무
