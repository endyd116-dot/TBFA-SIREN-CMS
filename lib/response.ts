...
export function ok(data: any = null, message?: string) {
  return new Response(
    jsonKST({ ok: true, message: message ?? null, data }),
...
export function created(data: any = null, message?: string) {
  return new Response(
    jsonKST({ ok: true, message: message ?? "생성되었습니다", data }),
...
export function badRequest(message = "잘못된 요청입니다", detail?: any) {
  return new Response(
    jsonKST({ ok: false, error: message, detail }),
...
export function unauthorized(message = "인증이 필요합니다") {
  return new Response(
    jsonKST({ ok: false, error: message }),
