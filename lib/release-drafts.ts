...
export const PENDING_DRAFTS: ReleaseDraftSeed[] = [
  ...
  },
  {
    ...
    items: [
      { text: "국민연금은 요율은 정률이지만 「기준소득월액 상한」이 있어 그 금액을 넘는 부분에는 보험료를 매기지 않습니다. 지금까지는 상한 없이 지급 총액에 4.5%를 그대로 매겨, 상여를 얹어 상한을 넘는 달에 과다 공제됐습니다(상여 500만원이면 약 99,000원)", link: "/cms-tbfa.html#payroll" },
      { text: "[관리자] 급여관리 › 급여 계산 기준에 «국민연금 상한액(월)» 칸이 생겼습니다. 현재 6,370,000원이며 매년 7월 공단이 조정할 때 이 칸만 고치면 됩니다. 0을 넣으면 상한 없이 계산합니다" },
      { text: "명세서에도 상한이 걸린 달은 계산방법이 «기준소득월액 상한 6,370,000원 × 4.5% (상한 초과분은 부과 제외)»로 표시돼, 직원이 왜 세전 총액 기준이 아닌지 바로 알 수 있습니다" },
      { text: "건강보험·고용보험·소득세는 종전과 같습니다. 4대보험은 누진이 아니라 정률이고, 상여가 있는 달에 공제가 크게 느는 것은 누진 구조인 소득세 때문입니다(연말정산에서 정산됩니다)" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { text: "확인서 직인 자리에 5월 시험 때 올라간 카카오톡 사진이 찍히던 것을 근로계약과 같은 이사장 도장(저장소 도장 파일)으로 바꿨습니다. 이미 발급된 확인서도 다시 열면 새 도장으로 다시 만들어집니다. [관리자] 다른 도장으로 바꾸려면 싸이렌 어드민 › 영수증 설정 › 직인 이미지에서 올리면 되고, 직인을 제거하면 저장소 도장이 기본으로 쓰입니다" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { text: "후원 랜딩(숭고한 등불)이나 우리 후원 창에서 후원회원으로 가입하는 순간, 「후원회원으로 등록되셨습니다 · 홈페이지에서 같은 휴대폰 번호로 인증하면 회원가입이 바로 완료됩니다 · 협의회 소식과 분기 등불 보고를 받아보실 수 있습니다」 안내가 카카오 알림톡으로 나갑니다. 카카오 검수(1~3영업일)가 끝나기 전에는 같은 내용이 문자로 나가고, 마이페이지 알림함에도 남습니다" },
      { text: "안내의 「홈페이지에서 가입 완료하기」 버튼은 홈 화면의 가입 창(휴대폰 인증)을 바로 엽니다. 인증하면 랜딩에서 만들어진 계정에 이메일·비밀번호가 붙어 마이페이지를 쓸 수 있습니다" },
      { text: "[관리자] 가입 뒤 첫 회비를 내지 않은 분께 3일·7일 뒤 문자로 안내하는 「등불 가입 · 미납 후속」 여정이 통합 CMS 💌 후원자 너처링 › 예비 후원자 탭에 생겼습니다(기본 OFF). 문구를 보시고 켜면 매일 아침 자동으로 나가며, 첫 회비가 확인되면 자동으로 멈춥니다. 소식 수신에 동의한 분에게만 갑니다", link: "/admin-nurture.html" },
      { text: "[관리자] 알림톡 템플릿 「후원회원 등록 안내(등불의 기적)」의 검수 상태는 통합 CMS 알림톡 템플릿 화면에서 보입니다. 문자·카톡용 짧은 주소 tbfa.co.kr/lantern(캠페인)·tbfa.co.kr/lantern/join(후원 창 바로 열기)도 생겼습니다" },
    ],
  },
  {
    key: "2026-09-06-payment-confirmation",
    title: "기부금 영수증 PDF가 「후원금(회비) 납부 확인서」로 바뀌었습니다",
    items: [
      { text: "협의회는 아직 공익법인(지정기부금단체) 지정 전이라 세액공제용 기부금영수증을 발급할 수 없습니다. 마이페이지에서 받는 PDF의 제목이 「후원금(회비) 납부 확인서」로 바뀌고, 소득세법 서식 각주 대신 「세액공제용 기부금영수증이 아니며 공익법인 지정 후 별도 발급」 문구가 들어갑니다", link: "/mypage.html" },
      { text: "확인서의 단체 표기는 영수증 설정(사단법인 교사유가족협의회 · 381-82-00754 · 대표 박두용 · 강서구 공항대로 426, 618호)을 그대로 씁니다. 항목 이름은 납부자 정보·단체 정보·납부 내역, 구분은 «특별회비(후원회원)»" },
      { text: "전환 전에 이미 발급된 PDF(옛 서식)는 다시 열 때 자동으로 새 서식으로 다시 만들어집니다(번호는 그대로). 파일 이름은 «납부확인서_번호.pdf»" },
      { text: "마이페이지 증명서 화면·후원 내역 표·해지 안내·감사 메일 버튼(「마이페이지에서 후원 내역 보기」)·연간 안내 알림(1월 15일) 문구도 «납부 확인서»로 맞췄습니다" },
      { text: "[관리자] 통합 CMS 영수증 설정에 저장돼 있던 옛 기본 문구(「기 부 금 영 수 증」·소득세법 각주)는 코드가 «설정 안 함»으로 보고 새 기본 문구를 씁니다. 직접 바꾼 문구는 그대로 존중합니다. 공익법인 지정 시 lib/pdf-receipt.ts 상단의 되돌릴 자리 주석 참고" },
    ],
  },
  {
    key: "2026-09-06-campaign-no-flicker",
    title: "캠페인 화면과 후원 창이 처음부터 완성된 모습으로 열립니다 (깜빡임 제거)",
    items: [
      { text: "「등불의 기적」 캠페인 페이지를 열면 밝은 기본 화면이 잠깐 보였다가 어두운 등불 화면으로 바뀌던 현상이 사라졌습니다 — 이제 서버가 등불 화면(실값·자주 묻는 질문 포함)을 완성해서 내보내고, 브라우저는 다시 그리지 않습니다", link: "/campaign.html?slug=%EB%93%B1%EB%B6%88%EC%9D%98-%EA%B8%B0%EC%A0%81" },
      { text: "「후원회원으로 함께하기」를 누르면 후원 창이 뜬 뒤 금액 칸·단계가 바뀌던 현상도 사라졌습니다 — 페이지가 열릴 때 창을 미리 등불 상태로 만들어 두고 회원 여부도 미리 확인해 두어, 누르는 순간 최종 화면(가입 단계 또는 금액 단계)이 바로 뜹니다" },
      { text: "다른 캠페인 페이지도 같은 방식으로 서버가 완성해 내보냅니다(제목·모금 현황·본문·후원 버튼). 홈·공지 등 일반 후원 창은 종전과 같습니다" },
      { text: "[관리자] 캠페인 실값 주소(/api/campaign-stats)에 모금 현황 4개 값(모인 회비 합계·목표·최근 후원자 30명 명단·집계 시각)이 추가됐습니다 — 후원 랜딩(숭고한 등불)의 게이지와 캠페인 페이지가 같은 숫자를 씁니다" },
      { text: "[관리자] 회원 병합 도구의 실행 결과에서 효성 청구 항목이 -1 로 표시되던 것을 정리했습니다(효성 청구는 회원 참조가 없어 이전 대상이 아님)" },
    ],
  },
  {
    key: "2026-09-06-external-register-fix",
    title: "외부 등록 — 신규 회원 일괄 등록이 실제로 저장되고, 효성 파일이 전화번호로도 회원을 찾습니다",
    items: [
      { text: "「신규 회원 일괄 등록」의 [등록] 버튼이 그동안 화면에서만 등록된 척하고 서버에는 저장하지 않던 옛 시연 코드였습니다. 이제 실제로 저장됩니다 — 전화번호가 같은 기존 회원은 새로 만들지 않고 메모만 덧붙입니다(결과 토스트에 신규·병합·오류 행 수)", link: "/cms-tbfa.html#external-register" },
      { text: "효성 계약정보·수납내역을 올릴 때 효성 회원번호가 없는 기존 회원(홈페이지 가입·수기 등록)도 전화번호로 자동 연결됩니다. 통과 때 같은 사람의 임시 계정이 또 생기던 문제가 사라집니다" },
      { text: "미확정 목록에 [선택 자동 재매칭] 버튼이 생겼습니다 — 계약정보보다 수납내역을 먼저 올려서 «신규»로 남은 행을 지금 기준(효성번호·전화번호)으로 다시 맞춘 뒤 통과하세요" },
      { text: "[관리자] 이미 생긴 중복 회원은 관리자 도구로 합칠 수 있습니다 — 주소창에 /api/admin-member-merge?from=임시계정id&into=원래회원id (미리보기) → &run=1 (실행). 효성 계약·수납·후원·알림이 원래 회원으로 옮겨지고 임시 계정만 지워집니다" },
      { text: "올바른 순서: ① 효성 계약정보 파일 업로드 → 통과(회원 생성·연결) ② 효성 수납내역 파일 업로드 → 통과(후원 내역 생성). 청구목록 파일은 올리지 않아도 됩니다. 신규 회원 일괄 등록은 효성이 아닌 명단(오프라인·옛 명단)에만 쓰세요" },
    ],
  },
  {
