...
| ★★★ | **응답폼·신청폼 빌더** | 단답·선택·파일업로드 폼을 운영자가 자유 작성. 행사 신청·설문·이벤트 수집·자격 검토 신청 등에 활용 | 7~10일 | 믹스온 기본 모듈, NPO 행사 운영 필수. SIREN은 캠페인 폼만 존재 | 회원/팬·발송 센터(완료 시 알림 연동) |
...
| 🐛 | **시스템 자동 발송 UI — 강제 채널 박스 조건 표시** | 현재 모든 이벤트 모달에서 "강제 발송 채널" 빨간 경고 박스가 항상 표시됨. forcedChannels가 비어있는 이벤트(workspace·briefing·support 등 7개)에서는 숨겨야 함. admin-system-notification.html buildForcedSection 분기 추가 | 15분 | 2026-05-16 Swain 보고 — A안 라운드 후 처리 | 없음 |
...
| 2026-05-16 | 메인 채팅 | §8 SaaS·외부 판매 준비 신규 추가 — 믹스온 비교 약점 4개 모듈 우선순위 명시 |
