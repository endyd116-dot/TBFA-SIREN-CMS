...
> **체크박스 패턴 (2026-05-12 도입)**: Sonnet 4.6용. 메인이 §3 화면 명세를 페이지·모달·동작 단위로 1:1 체크박스로 변환해서 박을 것.
...
{평행 mock인 경우} 다음 mock 응답 사용 (B 머지 후 메인이 실 API 안내, 응답 키는 §2.2와 1:1 일치):
  {예시 JSON}
...
**템플릿 마지막 갱신**: 2026-05-12 (§6.2·§6.3 체크박스 패턴 도입 — Sonnet 4.6의 명세 준수율 ↑, R2+R3 schema 격차 사고 차단)
