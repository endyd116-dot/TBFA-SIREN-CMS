...
모델: Sonnet 4.6 / 워크트리: ../tbfa-mis-B / 브랜치: feature/martyrdom-p2-back (베이스 origin/main @ {BASE_HASH})
■ 셋업(§6.0): cd ../tbfa-mis-B; git fetch origin; git checkout -B feature/martyrdom-p2-back origin/main; git log --oneline -1; git merge-base --is-ancestor {BASE_HASH} HEAD && echo 베이스OK || echo "⚠️어긋남-메인보고"
정독(필수): docs/active/2026-05-26-survivor-support.md §P2.0~§P2.2(+ 기존 §1·§2·§2.5·§2.8 토대). 참고: PARALLEL_GUIDE §3·§7
영역: netlify/functions/, lib/, db/schema.ts, drizzle/, package.json   금지: public/, assets/, PROJECT_STATE.md, docs/(상태는 메인에 보고 텍스트로만), lib/auth.ts·admin-guard.ts·hyosung-parser.ts 수정

[자율주행 정책 — CLAUDE.md §6.17]
- **git push 안 함(메인 단독)** — commit까지만, 완료 시 메인에 머지 요청(브랜치·해시·변경 요약). force push·hard reset·rm -rf 금지
- 파일 읽기·수정·git(push 제외: status/log/diff/add/commit/rebase)·bash·PowerShell·npm install/run은 묻지 말 것(자율)
- 묻기: 설계·로직 결정 / package.json·lock 수정(신규 의존성 추가 시) / npm uninstall·update / netlify·curl
- 막히면 즉시 보고(30분 이상 혼자 헤매지 말 것)

[진행률 보고 의무 — CLAUDE.md §6.16] 큰 단계(체크박스 1개) 완료마다 "📊 진행률 X% (n/17 완료) — 다음: ..." 한 줄. 매 응답마다 ❌
[코드 표준 — code_standards] background 함수 **config.path 금지**(자동체인 멈춤 근본원인)·INTERNAL_TRIGGER_SECRET fail-closed / requireAdmin→auth.res·super_admin은 역할 확인 / drizzle 다중 leftJoin 금지(separate query+Map) / embedding은 raw SQL만 / **정량 % 일치율 출력 금지**(정성+근거만·§8) / AI 호출 featureKey:"martyrdom_ai"·internalBulk:true·timeoutMs 넉넉히

━━━ §P2.1 DB 체크리스트 ━━━
□ schema.ts append-only(파일 끝 /* === P2 순직 인정 지원 (2026-05-26) === */ 헤더) — martyrdom_deadlines·martyrdom_criteria·martyrdom_actions 3테이블 + martyrdom_case_documents.evidence_strength + martyrdom_cases.consent_note·consent_obtained_at (마이그 적용 전엔 정의 보류)
□ import 점검: serial·integer·varchar·text·date·timestamp·boolean·index 누락 없이
□ migrate-martyrdom-p2.ts (admin·GET?run=1·진단 GET·멱등 IF NOT EXISTS — 3테이블+인덱스 / ALTER 컬럼 ADD IF NOT EXISTS / martyrdom_criteria 기본 요건 8종 시드 ON CONFLICT(code) / 환경변수 MARTYRDOM_STORAGE_ALERT_GB 안내)

━━━ §P2.2 lib·API 체크리스트 ━━━
□ lib/martyrdom-ai.ts — analyzeStrategy(caseId,caseKind): searchRag(q,k,['martyr_case','martyr_law'],caseId) 다중쿼리+extractionJson → strategy JSON(③가능논리·부족자료·쟁점·causalChain·similarCases + ⑨conflicts(치명/주의)·⑩masterTimeline(gap)·⑪counterArguments·ragSources) **1콜 통합**
□ lib/martyrdom-ai.ts — buildGoldenAdvice(caseId)(golden_items master+정황→우선순위·맞춤) / checkCriteria(caseId)(martyrdom_criteria+extractionJson+RAG→met/partial/unmet+근거) / computeReadiness(caseId)(**규칙 계산** 요건40·증거30·타임라인15·모순15 + AI 첨언 1콜·정성 note) / learnFromClosedCase(caseId)(recognitionPattern→martyr_case 색인)
□ classifyDocument에 evidenceStrength(강/중/약) 판정 추가(분류와 동시·1콜 내·기존 응답키 유지)
□ admin-martyrdom-generate (POST·{caseId,type})·디스패처: strategy/golden/criteria→status processing+generate-background 트리거 / readiness→inline 계산 응답
□ admin-martyrdom-generate-background (INTERNAL·**config.path 금지**·type별 lib 호출→ai_outputs INSERT version++·status draft)
□ admin-martyrdom-readiness (POST·규칙 % + breakdown + gaps + AI 첨언 → ai_outputs(readiness))
□ admin-martyrdom-output-review (PATCH·{outputId,status:reviewed|discarded,reviewNote})
□ admin-martyrdom-criteria (GET/POST/PATCH/DELETE·쓰기 super_admin) / admin-martyrdom-criteria-generate (super_admin·법령 docs/law/martyrdom AI 파싱→요건 후보 제안)
□ admin-martyrdom-deadlines (GET/POST/PATCH/DELETE) / admin-martyrdom-actions (GET/POST/PATCH/DELETE·source=missing_evidence|manual)
□ admin-martyrdom-dashboard (GET·G3 다중사건 집계: 기한임박·준비도·상태 + 저장용량 sum(size_bytes))
□ admin-martyrdom-corpus-search (POST·searchRag ['martyr_case','martyr_law']만·active 제외)
□ cron-martyrdom-deadline (scheduled·netlify.toml 등록·KST 08:00·D-7/소멸시효 임박 알림 + 저장용량 MARTYRDOM_STORAGE_ALERT_GB 초과 알림·기존 notifications/notifyAdmins 재사용·중복 알림 방지 alertedAt)
□ admin-martyrdom-close-learn-background (INTERNAL·config.path 금지) + admin-martyrdom-cases PATCH에 status→closed+outcome 시 트리거 추가(⑥)
□ (수정) admin-martyrdom-analyze-background: extraction 후 active 사건이면 generate-background(type=strategy) **자동 트리거 추가**(자동 체인 연장·디바운스)

━━━ 응답 구조 (A mock=§P2.2 계약과 키 1:1·1글자도 변경 금지) ━━━
{발사 시 §P2.2 strategy/criteria_check/readiness/golden JSON 전문 임베드}

━━━ push(=메인 머지) 전 체크 ━━━
□ export const config = { path } 각 /api 함수(background은 config 없음) / requireAdmin→auth.res·super_admin 역할 확인
□ background INTERNAL_TRIGGER_SECRET·fail-closed·throw 안 함 / embedding raw SQL만 / 정량% 금지 준수
□ npx tsc --noEmit 통과 / 마이그 적용 전 schema 신규 정의 활성화 금지(헤더만)
□ 완료 보고: 브랜치·해시·변경 파일 + 체크박스 전체 체크 한 줄 + schema 격차 적응 시 별도 표
```
...
모델: Sonnet 4.6 / 워크트리: ../tbfa-mis-A / 브랜치: feature/martyrdom-p2-front (베이스 origin/main @ {BASE_HASH})
■ 셋업(§6.0): cd ../tbfa-mis-A; git fetch origin; git checkout -B feature/martyrdom-p2-front origin/main; git log --oneline -1; git merge-base --is-ancestor {BASE_HASH} HEAD && echo 베이스OK || echo "⚠️어긋남-메인보고"
정독(필수): docs/active/2026-05-26-survivor-support.md §P2.3(+ §P2.0 결정). 참고: PARALLEL_GUIDE §3
영역: public/, assets/   금지: lib/, netlify/functions/, db/, drizzle/, PROJECT_STATE.md, docs/
모드: 평행(아래 mock으로 시작·B 머지 후 실 API 자동 전환)

[자율주행 정책 — CLAUDE.md §6.17] git push 안 함(메인 단독)·commit 후 머지 요청 / 파일·git(push제외)·bash·npm install 자율 / 설계·로직만 묻기 / 막히면 즉시 보고
[진행률 보고 의무] 큰 단계 완료마다 "📊 진행률 X% (n/12 완료) — 다음: ..." 한 줄
[코드 표준 — code_standards #3] public/js/*.js는 **순수 JS**(as·interface·제네릭<T> 등 TS 문법 금지) / 커밋 전 **node --check <file>** / api()는 credentials:'include'·이중 stringify 금지 / 응답키 다중 fallback(res.data.data.X||res.data.X||res.X) / 캐시버스터 ?v=P2

━━━ mock (B 머지 전·§P2.2 계약·키 1글자도 변경 금지) ━━━
{발사 시 §P2.5 strategy/criteria_check/readiness/golden mock 전문 임베드}
...
□ ③분석 탭 모순(⑨) 카드(치명/주의 배지) + 마스터 타임라인(⑩·공백 회색) + 예상 반론(⑪) 카드
...
━━━ push(=메인 머지) 전 체크 ━━━
□ mock 키명 = B 응답(§P2.2)과 1:1 / 모든 변경 JS node --check 통과 / <script> 캐시버스터 ?v=P2 / public/ 외 변경 0
...
모델: Opus 4.7 / 워크트리: ../tbfa-mis-C / 브랜치: verify/martyrdom-p2 (베이스 origin/main @ {B·A 머지 후 해시})
■ 셋업(§6.0): cd ../tbfa-mis-C; git fetch origin; git checkout -B verify/martyrdom-p2 origin/main; git log --oneline -1
정독: docs/active/2026-05-26-survivor-support.md §P2.4. 참고: PARALLEL_GUIDE §7·§8
...
작업 순서:
  ...
  2) 핵심 점검: 추출→전략 자동 체인 작동 / 준비도 규칙 % **재현성**(같은 입력=같은 값) / 정량% 미노출 / 코퍼스 검색 active 제외 / cron 중복 알림 방지(alertedAt) / 저장용량 알림 임계
  3) §P2.4 회귀 — P1(추출·분류·삭제·재처리·진행오버레이·음성영상 전사) / 기존 RAG(ai_rag_search) / 어드민 로그인·CMS 기존 탭 / 비용 안전장치 martyrdom_ai 합산
  4) BUG 발견 시 fix 커밋(verify 브랜치 그대로) → 메인 보고 (역할 전환 fix는 PARALLEL_GUIDE §17)
  5) 보고서 docs/history/verify/2026-05-26-martyrdom-p2.md
표현 규칙(CLAUDE.md §6.14): 함수·코드 용어 없이 사용자 동작·결과 위주
[자율주행 정책] git push 안 함·commit 후 메인 보고  [진행률 보고 의무] 큰 단계마다 한 줄
```
