...
1. **postback 시크릿** — 값 수령 전(위 ③).
2. **회칙(정관) 링크** — 홈페이지에 정관 문서 페이지가 없어 가입 폼의 「회칙(정관)」 링크가 임시로 자료실(`/resources.html`)을 가리킨다. 정관 PDF/페이지 주소가 정해지면 `LANTERN_BYLAWS_URL` 환경변수 한 줄로 교체.
3. **사이트 공통 푸터의 사업자번호**는 118-82-71215 그대로(요청 범위가 「등불의 기적」 페이지·영수증 표기라 그 페이지에서만 381로 덮어씀). 사이트 전체를 381로 바꿀지는 사장님 결정.
