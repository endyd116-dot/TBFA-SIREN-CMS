...
## 3. 열린 작업 / 다음 단계
### (P1) 카톡 알림톡 우회 — ✅ 구축·검수 제출 완료(2026-06-26)
- **구축됨**: "소식 도착 알림"(event_key=NURTURE_NOTICE) 1종을 **교사유가족협의회 채널**에 등록 → **카카오 검수 제출(inspecting)**. solapiTpl `KA01TP260625233336650v2kqO4tBhpm`. 승인되면 `cron-kakao-template-status`(매시간)가 자동 `approved` 전환.
- **엔진 라우팅**(`lib/kakao-nurture-notice.ts`): 카톡 단계일 때 승인템플릿 있으면 알림톡(소식 도착 알림)+실제 본문 SMS 대체발송(text), 없으면(검수 전·반려) **일반 문자로 자동 강등** → 내용 미수신 0.
- **B안 인앱**(`lib/nurture-engine.ts deliverNurtureInApp`): 모든 너처링 발송을 마이페이지 알림함(`/mypage.html#notifications`)에도 createNotification 기록. 알림톡 '확인하기' 버튼이 이 알림함을 염. 문자·메일은 흘러가도 인앱은 남아 신뢰·브랜드.
- ⚠️ **솔라피 카카오 채널 2개**(교사유가족협의회+함께워크on) — 채널 선택 `[0]` 금지. 이름 매칭(교사유가족|협의회|tbfa)→SOLAPI_KAKAO_PFID→[0]. 등록함수 + `admin-kakao-templates.ts` 둘 다 fix(커밋 e50bf40).
- **남은 것**: 카카오 승인 대기(외부·1~2일·정보성 판정은 카카오 권한). 친구톡(마케팅 카톡)은 채널 친구 확보 선행. 1회용 등록함수는 호출 후 삭제 완료.
