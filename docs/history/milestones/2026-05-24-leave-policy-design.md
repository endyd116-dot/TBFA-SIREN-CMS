...
## 구현 결과 (2026-05-24 · Swain 수면 중 자율 진행)

### 데이터 모델 (Swain 확정)
- **`att_policies`(근무 정책 기본행 is_default=true) 확장** 채택 — 신규 테이블 대신. 이미 슈퍼어드민 전용 저장 경로(admin-att-policy)·단일 기본행·DB 시드가 갖춰져 신규 테이블은 중복.
  - 추가 6컬럼: `leave_accrual_mode`('A'/'B') · `annual_base_days`(12) · `annual_increment_days`(1) · `annual_increment_years`(2) · `annual_cap_days`(25) · `perfect_bonus_per_month`(1)
- **`members.hire_date`(date·NULL)** 같은 마이그에 추가 — 모드 B 근속 계산용. NULL이면 가입일(`created_at`) 폴백.
- 마이그 `migrate-att-leave-policy` 적용 완료(2026-05-24·**appliedCount 7**) → schema 정의 활성화 + 마이그 파일 삭제.

### Stage 별 완료
- **Stage1**: 마이그 함수 + schema 활성화 ✅
- **Stage2-1**: `admin-att-leave-policy`(GET/PUT·super_admin=`ctx.member.role`·없으면 UPSERT 시드 P1-17) ✅
- **Stage2-2**: `cron-att-leave-auto` 정책 로드 → 모드 분기 재작성 ✅
  - 모드 A: 전월 만근 → +`perfect_bonus_per_month`일 (Section 2 미실행)
  - 모드 B: 입사월=당월·1주년↑ → `base + floor((근속년수-1)/incYears)*incDays`(상한 cap), ON CONFLICT GREATEST로 기존 잔여 보존 (Section 1 미실행)
  - 임시 가드 `SERVICE_BASED_ANNUAL_ENABLED` 제거 → 정책값으로 대체
- **Stage3**: 근태 설정 화면 '근무 정책' 탭에 연차 산정 정책 섹션(모드 라디오 + 모드별 파라미터·토글) + 캐시버스터 `?v=17-leavepol` ✅

### 카드 만료 부록 (방식② 구현 완료)
- `billing-card-expiry-set`(POST) 신규: billing-success에서 유효기간(MM/YY) 입력 → `donationId→billingKeyId→billing_keys.card_expiry_month`(YYMM 4자리) **raw SQL UPDATE**.
- ⚠️ **발견**: `billing_keys.card_expiry_month`는 schema.ts 정의엔 **없으나 DB엔 존재**(cron이 운영 중 raw SQL로 SELECT). schema 정의 추가 시 billing_keys SELECT 회귀 위험 → **cron과 동일하게 raw SQL로만 접근**(schema 비의존·완전 격리). 트리거의 "컬럼 이미 존재" 전제는 맞음.
- billing-success.html에 유효기간 입력 박스(선택)·핸들러 추가.

### 검증
- tsc 0 (clean). 코드 회귀 점검: schema-DB 일치·billing_keys 격리·근무/연차 저장 충돌 없음·근속 계산 시나리오 일치(1주년12/3주년13/5주년14).
- ⏳ **Swain 라이브 검증 대기** (수면 후): ① 근태 설정 → 근무 정책 탭에 연차 정책 섹션 노출(슈퍼어드민) ② 모드 A/B 라디오 토글·저장·재로드 ③ billing-success에서 유효기간 입력·저장.

### 남은 것 (TODO·모드 B 실사용 전)
- **회원 상세에 입사일 입력 칸 추가** — 현재 모드 A(5인 이하)라 미구현. 모드 B 전환(5인 초과) 시 정확한 근속 계산 위해 필요. 그 전까지는 가입일 폴백으로 동작(Swain 확정).
