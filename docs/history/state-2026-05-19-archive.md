...
| 2026-05-19 | **메인** | **✅ 라운드10 + 워크스페이스 버그4건 C 검증 완료** (main @ 4a1d5ca) — R10 12/12 PASS, 버그4건 12/12 PASS. BUG 없음 |
| 2026-05-19 | **메인** | **✅ Phase 26 근태관리 완결** (main @ 69cddd4) — C 검증 20/21 PASS + BUG-P26-1 fix. 라운드10 A+B 머지 완료(2d90461), C 검증 트리거 발사 |
...
## 2.5. 로드맵 결정 (2026-05-20 Swain)

| 후보 | 결정 | 시점 |
|---|---|---|
| **Phase 23 재정 v3.0** | 🟢 진행 — **최소 3개월 뒤 (2026-08-20 이후)** | 합의 세션 별도 |
| **Phase 20 Light 운영 보강** (장애 알림·healthcheck cron·백업 정책) | 🟡 후순위 — 무기한 보류 | 필요 시점 즉시 발사 가능·B 단독 4~5h |
| **Major 의존성 갱신** (drizzle·typescript·zod·bcryptjs 등) | 🟡 후순위 | 운영 위험 시 또는 R37+ |
| **6순위 #8 1:1 매칭 채팅** | ✅ 완료 (2026-05-11) | — |
| **카카오 알림톡 자동화** | 🟢 운영 액션 — Swain Aligo 환경변수 등록 | 심사 통과 후 |
| **base_salary 운영 데이터 입력** | 🟢 운영 액션 — UI 완비·실 입력만 | Swain 직접 |

→ R35-GAP round4 + R36-Cleanup 완료까지가 명세 정합 시리즈 종결 시점. 이후 큰 라운드 없이 **운영 모드**.
...
| 채팅 | 영역 | 작업 항목 | 상태 |
|---|---|---|---|
| 메인 | 설계·머지 | DB 마이그 불필요, 기존 테이블 활용 | ✅ 완료 |
| B | 백엔드 | comment-vote·comment-report·admin 검토 + 파일 공유 + 알림 구독 + AI 법률 배정 7개 | ✅ 완료 (머지됨) |
| A | 프론트 | incident.js 투표/신고 UI + workspace-files.js 공유 모달 + settings-notifications.js | ✅ 완료 (머지됨) |
| C | 검증 | Q1~Q12 라이브 검증 | ✅ 완료 (12/12 PASS, BUG 0) |
...
| **Phase 26 근태관리 시스템** | ✅ 100% 완결 (2026-05-19, main @ 69cddd4) — A+B 머지·마이그 완료·C 검증 20/21 PASS + BUG-P26-1(notifications 사이드바 링크 누락) fix |
| **Phase 27 근태관리 Step9~17** | ✅ 100% 완결 (2026-05-19, main @ 0af70d6) — 재택보고서 API·AI 초안·cron 4개·att-checkin/checkout 수정 |
| **Phase 28 성과관리 완성** | ✅ 100% 완결 (2026-05-19, main @ a3d8fc3) — EVENT_RANGE UI·비매출 배지·AI 코칭·급여 CSV·알림 11종·AI 2가지·cron. C 10/10 PASS |
| **라운드 10 댓글·공유·알림·AI 법률** | ✅ 100% 완결 (2026-05-19, C 12/12 PASS) |
