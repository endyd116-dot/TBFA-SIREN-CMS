...
> **▶ 최신 세션 인수인계 = [`docs/active/2026-09-06-lantern-session-handoff.md`](../active/2026-09-06-lantern-session-handoff.md)(§6 = 깜빡임 FIX 결과) + 트리거 [`docs/active/2026-09-06-RESUME-TRIGGER.md`](../active/2026-09-06-RESUME-TRIGGER.md) — 새 메인은 트리거부터 정독.** 깜빡임 FIX 완료 → 다음 = 영수증 PDF→「후원금(회비) 납부 확인서」(트리거 §2).
...
## 0-A. ★ 지금 시점 (2026-07-20 최신) — 지출결재·지출결의서 트랙

사무실 세션(7/16~7/20)에서 **지출 결재 워크플로우를 실사용 가능한 수준까지** 끌어올림. 전부 main 배포 완료.

### 7/16 — 결재 기능 확장
- **증빙 첨부 + 결재라인 CRUD**(이사장 전용) — 결재선을 화면에서 직접 만들고 고침(코드 변경 불필요·§6.18 원칙 준수).
- **지출결의서**: 결재취소·기안삭제·발행본 일괄다운로드·검색·CSV.
- **반려건 재상신** + **결재 지연 리마인드 cron**(`cron-approval-delay-reminder`).
- **예산과목(관/항/목) CRUD 버튼 전부 무반응** — 함수명 불일치 fix.
- **협의회 소개 페이지 시드** 마이그 추가(`migrate-about-content`).
- 푸터 협회 정보가 헤더 메뉴 요청 완료를 불필요하게 기다리던 지연 제거.

### 7/20 — 증빙·PDF 서식 fix
- **증빙이 안 열리던 버그**(P1): 업로드 3단계(주소발급→R2 PUT→**완료확정**) 중 3단계 누락 → `blob_uploads.upload_status`가 `pending`으로 굳어 `blob-image`가 425로 서빙 거부. 파일 자체는 R2에 정상. `admin-approval.js`·`admin-expenses.js` 두 업로드 함수에 `blob-confirm` 호출 추가. 굳은 4건은 1회용 마이그로 복구 완료(`healedCount:4`·파일 삭제됨).
- **지출결의서 PDF**: 자기결재칸 제거(기안자가 결재선 직책 겸직 시)·기안일 표기·**본문 표 넘침 fix**·발행본 **재발행** 엔드포인트(이사장 전용·내용 불변·서식만 갱신). 생성 로직은 `lib/resolution-pdf-store`로 승인·재발행 공용화(드리프트 방지).

### ⚠️ 재발 주의 (두 트랙 공통 함정)
1. **R2 업로드는 3단계가 세트** — 완료확정(`blob-confirm`)을 빠뜨리면 파일은 올라가는데 열리지 않고, **업로드 시점엔 에러가 안 나서 못 잡는다.** 신규 업로드 경로 작성 시 3단계 전부 있는지 확인.
2. **PDF 표 넘침 = 급여 PDF와 같은 뿌리** — 한글 폰트 통임베드 시 `/DW` 부재로 숫자·공백이 전각(1em)으로 벌어짐 → 폭 계산과 렌더가 어긋나 칸을 넘침. **글자별 직접 배치(`drawRun`)** 로 우회. 새 PDF 만들 때 같은 함정 반복 주의(`lib/payroll-pdf.ts` 상단 경고 주석).

### ▶ 미결 — Swain 액션
1. **🔴 간이지급명세서 제출 — 기한 7/31**(§0-B 4-2와 동일·아직 미제출이면 최우선).
2. **🔴 관리자 비밀번호 변경**(git 히스토리 노출 이력·변경만이 해결).
3. **🟡 협의회 소개 시드 마이그 호출 여부 확인** — `netlify/functions/migrate-about-content.ts` 잔존. 호출했으면 파일 삭제(다음 배포 동봉), 미호출이면 `https://tbfa.co.kr/api/migrate-about-content?run=1`.
4. **🟡 잔존 1회용 마이그 2개**(`migrate-content-perm`·`migrate-r44-reindex`) — 7/13 세션 이전 잔여. 호출 여부 확인 후 삭제 판단.
5. 김광일 6월 명세서 마무리 / 업데이트 소식 초안 발행 — 상세는 7/13 세션 핸드오프 §4.

### 📌 문서 상태 메모
- `PROJECT_STATE.md` §2는 **2026-05-29에서 멈춰 있음**(R45 기준). 7월 트랙(급여·KST·지출결재)은 반영 안 됨 — 본 §0-A + 7/13 세션 핸드오프가 실질 최신 출처.
- `APP_VERSION` = `2026-07-20.1`. 7/16~7/20 소식 초안 7건 추가됨(발행은 운영자 수동).

---

## 0. 지금 시점 (2026-05-27) — archive
