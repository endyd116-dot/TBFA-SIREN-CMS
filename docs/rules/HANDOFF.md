...
### 🟢 [다음 메인 과제·새 세션] 연차 산정 정책 기능
운영 전 검수 P1-14에서 파생. 슈퍼어드민이 연차 산정 방식을 선택·CRUD:
- **★ 설계서(단일 출처) = [`docs/active/2026-05-24-leave-policy-design.md`](../active/2026-05-24-leave-policy-design.md)** — 8섹션(데이터모델·설정API·cron분기·어드민UI·마이그 흐름·단계) + **카드 만료 유효기간 입력 부록**(방식② 추천). 새 세션은 이 문서 정독 후 Stage1부터.
- **모드 A (5인 이하)**: 월 만근 시 +1 유급휴가 — **현재 협회 = 5인 이하라 이 모드.**
- **모드 B (5인 이상)**: 근속 기반(기본 1주년 12일·2년마다 +1·기준일수/증가일수/증가주기/상한 설정).
- 필요: 정책 설정 스키마+마이그(Swain 호출)·슈퍼어드민 설정 UI·`cron-att-leave-auto` 모드 분기 재작성·hire_date 컬럼 검토.
- **현재 임시 상태**: cron-att-leave-auto가 모드 A(만근 +1)만 운영(`SERVICE_BASED_ANNUAL_ENABLED=false`로 근속 1주년 일괄부여 비활성). 데이터 손실 버그는 해소됨.
