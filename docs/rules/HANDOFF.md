...
### 🔴 [최우선·다음 메인 과제] 운영 전 전수 검수 라운드 (2026-05-23 설계)
오픈 전 전 기능 전수 점검 — 워크플로우 단절·로직 미연결·업데이트 누락을 메인/A/B/C 4분할(도메인별)로 검수.
- **결과물 = 발견사항 리포트 + 심각도(P0/P1/P2)만. 검수 중 코드 수정 금지**(수정은 합의 후 별도 라운드).
- **★ 설계서 = `docs/active/2026-05-23-prelaunch-audit.md`** (5축 방법론·도메인 4분할·출력형식·트리거). 새 메인은 이걸 정독 후 A/B/C 트리거 발동·리포트 취합.
- 도메인: 메인(후원·결제·인증·알림+총괄) / A(SIREN신고·유족지원·공개사이트) / B(워크스페이스·근태·성과·급여·AI) / C(어드민CMS·재정·권한·빌더·인프라).
