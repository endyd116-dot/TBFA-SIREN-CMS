...
### 🟢 [다음 메인·즉시] ④ 교유협 뉴스 분석
③ 완료. ④는 **새 메인 세션에서 설계부터** (네이버 검색 키·cron·워드클라우드·히스토리·diff = 최대 규모).
- **★ 단일 출처 = [`docs/active/2026-05-24-milestone-and-news.md`](../active/2026-05-24-milestone-and-news.md) §④** — 요구·결정(네이버 수집+Gemini 분석)·접근·미결정.
- **착수 전 Swain env**: `NAVER_SEARCH_CLIENT_ID`·`NAVER_SEARCH_CLIENT_SECRET`(네이버 개발자센터 검색 API) 발급·Netlify 등록.
- 라운드: 설계서(PARALLEL_TEMPLATE) → Swain 데이터모델/키 확인 → 마이그(Swain 호출) → 구현 → C 검증.
