...
### 🔜 다음 작업 — 근태 메뉴 재배치 (설계·흡수 가이드 완비 / 구현 대기)
Swain 요청: 근태 조회 탭은 메인, 설정 탭만 안쪽. cms 사이드바 "근태관리 설정" 1개 → **"🟢 근태 현황"(조회 8탭)·"⚙ 근태 설정"(설정 4탭)** 2메뉴. 두 화면(workspace-management + attendance-settings) 탭을 흡수·통합 후 `?group=ops|config` 필터.
- **설계+흡수 구현 가이드 완비**: `docs/active/2026-05-21-att-menu-reorg.md` §6 (흡수 대상 함수·충돌 rename·toast 호출부·group 필터·cms 라우팅 전부 명시).
- 큰 작업·회귀 위험 → **새 세션에서 집중 구현 권장**. 구현 후 Swain 라이브 검증.
- 분류: 조회=records·leaves·monthrecords·balances·workmodeChanges·remotereports·schedule·workmodes / 설정=policy·holidays·leavetypes·workplaces. (직원스케줄 경계는 검증 때 조정 가능)

### 그 다음 — R40 PG 전환 (토스→KICC)
Swain "작은 작업 다 끝내고 KICC". 메뉴 재배치 후 착수. kicc.md 정독 + 기존 토스 회원 처리·옵션 결정 필요.
