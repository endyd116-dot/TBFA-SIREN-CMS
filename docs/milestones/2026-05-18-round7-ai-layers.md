...
Phase 1 (동시): Layer 1 + Layer 2
  A 워크트리 (Opus)   → Layer 1 배치 도구 4개
  B 워크트리 (Sonnet) → Layer 2 파이프라인 도구 2개

Phase 2 (동시, Phase 1 머지 후):
  C 워크트리 (Opus)   → Layer 3 힌트 시스템
  D 워크트리 (Sonnet) → Layer 4 스케줄 도구
```
...
Phase 2 완료 후:
  git merge feature/layer3-hints    (C)
  git merge feature/layer4-schedule (D)
  → Layer 3: ToolResult 인터페이스 + 핸들러 수정
...
| 채팅 | 모델 | 브랜치 | 워크트리 |
|---|---|---|---|
| A (Layer 1) | Opus | `feature/layer1-batch` | `../tbfa-mis-A` |
| B (Layer 2) | Sonnet | `feature/layer2-pipeline` | `../tbfa-mis-B` |
| C (Layer 3) | Opus | `feature/layer3-hints` | `../tbfa-mis-C` |
| D (Layer 4) | Sonnet | `feature/layer4-schedule` | `../tbfa-mis-D` |
