...
이번 R4 핵심:
  - schema 컬럼 추가 4개 (workspace_memos 3 + admin_users 1)
  - 업무 템플릿 10종 시드
  - 자연어 검색 신규 함수 (Gemini 호출 + JSON 필터)

작업 순서:
  1) schema.ts 끝에 §1.2 컬럼 정의 추가 (주석 상태)
     ※ import 라인: time 누락 점검 + 추가
  2) 마이그 함수: netlify/functions/migrate-phase21-r4.ts
     - GET ?run=1 + requireAdmin (super_admin)
     - §1.3 SQL 그대로 (ALTER + INSERT 시드)
     - 진단 모드: 컬럼 존재 + 템플릿 count 응답
  3) lib/natural-search.ts 신규 — Gemini 호출 + JSON 파싱 (markdown fence 제거 + timeout 10s)
  4) admin-workspace-task-search.ts 신규 — POST 자연어 검색
  5) admin-workspace-memos.ts 수정 — eventDate/eventTime/showInCalendar 필드 처리
  6) admin-workspace-events.ts 수정 — includeMemos=1 파라미터 추가
  7) admin-user-preferences.ts 수정 — defaultWbsView 필드 추가 (R2+R3에서 만든 함수에 필드만)
  8) admin-workspace-tasks.ts 수정 — feed=1 응답에 actorName·groupKey 보강
  9) `npx tsc --noEmit` 통과
 10) push → 메인 보고 (마이그 호출 URL 포함)

머지 전 체크:
  - export const config = { path }
  - requireAdmin 반환 auth.res
  - AI 호출 실패 fallback (빈 결과·friendly 메시지)
  - JSON 파싱 try/catch
  - schema import (time) 누락 0
...
작업 순서:
  1) public/js/workspace-memo-modal.js (신규) — 메모 작성·수정 모달 (캘린더 표시 체크박스 + 시간 입력)
  2) public/js/workspace-calendar.js 확장:
     - YIQ 명도 계산으로 텍스트 색 자동
     - dateClick 핸들러 → "업무/일정/메모" 3옵션 팝업
     - loadEvents에 includeMemos=1 + 메모 row 렌더 (아이콘 구분)
  3) public/js/workspace.js 확장:
     - 활동 피드 자연어 + 시간 그룹핑(오늘/어제/이번주/이전)
     - 권한 기반 클릭 이동
     - 메모 패널 "새 메모" 버튼 → WorkspaceMemoModal.openCreate()
  4) public/js/workspace-kanban.js 확장:
     - 헤더에 보기 모드 토글(보드/리스트/캘린더) — defaultWbsView 자동 로드
     - 리스트 모드 렌더 + 행 클릭 → 카드 모달
     - 캘린더 모드 렌더 (FullCalendar 임베드)
     - 검색바 + "🤖 AI 검색" 버튼 + 해석 결과 표시
  5) public/js/workspace-activity-render.js 확장 (R1에서 도입) — 신규 actionType 매핑
     (task.transfer · task.assign · service.assignee_change · service.closed)
  6) public/js/mypage-out-of-office.js 확장 → 기본 보기 모드 카드 추가 (또는 별도 모듈)
  7) public/css/workspace-kanban-views.css (신규) — 리스트·캘린더 뷰 스타일
  8) workspace.html · workspace-kanban.html · workspace-calendar.html · mypage.html — 신규 스크립트·CSS <script>/<link>
  9) §3.5 캐시버스터 갱신
 10) 화면 진입·각 시나리오 동작 확인 후 push

머지 전 체크:
  - §3.2 와이어프레임 필드·버튼 존재
  - §3.4 토스트 문구 정확 일치
  - public/ 외 파일 변경 0
  - 보기 모드 전환 시 localStorage + 서버 동기 (debounce 1s)
  - AI 검색 실패 시 키워드 검색으로 자동 fallback
