...
>
> **마지막 갱신**: 2026-05-10 / 메인 채팅 / Phase 1 (#16 단계 B) 완료 시점
...
- ✅ #BUG-1 해결 (커밋 `bb529f9`)
- 🟡 다음 본격 작업: **마일스톤 #16 단계 C** (3~4h) — 후원 회원 분류(정기/잠재/비후원) 정착

누적 마스터플랜 진행률 약 35% (5.5/22 Phase + 4·5순위 + 6순위 #6·#15 + #16 단계 A·B).
...
| **B** 통합 일반 회원 + 상세 모달 | ✅ **완료 (2026-05-10)** | 실제 ~3h | 가짜 회원 제거 + 진짜 명단 + 5종 가입경로 뱃지 + 상세 모달(기본 정보/후원 내역). **#BUG-2 해소** |
| **C** 후원 회원 분류(정기/잠재/비후원) 정착 | 🟡 **다음 진입 예정** | 3~4h | 회원 테이블에 분류 컬럼 추가 + 토스 결제 발생 시 즉시 반영 + 효성 CSV 업로드 시 일괄 반영 + 정기/잠재 화면 본격 |
| D CSV 종합 검증 강화 + 자동 상태 전이 | ⬜ 미착수 | 3~5h | 효성 결제 데이터 파서 추가 + 야간 자동 동기화 작업 + 종합 대시보드 |
...
| 일시 | 내용 |
|---|---|
| 2026-05-10 | **Phase 1 완료** — 단계 B(통합 회원 + 상세 모달) 머지·검증 통과. #BUG-2 해소. tag `phase1-complete-20260510`. 다음: 단계 C 진입 |
