...
| #8 변호사·심리상담사 1:1 매칭 채팅 | ✅ main 머지 (`fe84ed9`) | 🟡 Swain 검증 대기 |
| #9 관련 사이트 | ✅ | ✅ |
| #10 정기후원 해지 안내 | ✅ | ✅ |
...
**V1** — 사용자: 마이페이지 → ⚖️ 전문가 상담 탭 → 신청 (유형·도메인·사유) →
어드민: 사이드바 ⚖️ 1:1 매칭 관리 → 대기 목록 확인 → 전문가 배정 → 채팅방 자동 생성

**V2** — 사용자 채팅방 진입 + 메시지 전송 → 전문가 계정으로 확인 및 답장

**V3** — 어드민 → 진행중 탭 → 세션 종료 → 사용자 마이페이지 완료 탭 확인
